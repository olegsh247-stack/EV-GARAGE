// ЭТО ДЕМО-ДАННЫЕ. Реальные фото, характеристики и цены нужно будет
// заменить на настоящие через этот файл (или через будущую админку).
// Формат гибкий — можно добавлять сколько угодно брендов, моделей и версий.
//
// Порядок полей в Trim соответствует порядку показа характеристик на
// странице версии — примерно как на китайских каталогах: сначала что за
// автомобиль и энергоустановка, потом запас хода и динамика, потом батарея
// и зарядка, потом привод и вместимость.

export type PowertrainType = "BEV" | "EREV" | "HEV" | "PHEV";

export const POWERTRAIN_LABELS: Record<PowertrainType, string> = {
  BEV: "чистый электромобиль",
  EREV: "последовательный гибрид",
  HEV: "гибрид",
  PHEV: "подключаемый гибрид",
};

export type Trim = {
  slug: string; // для URL версии, напр. "550-pro"
  name: string; // отображаемое название версии/комплектации
  priceFrom: number; // цена в рублях (ориентировочно, нужно подтверждать)
  priceCny?: number; // реальная цена в юанях, если известна — для точного расчёта таможни
  powertrainType: PowertrainType;
  motorModel?: string; // модель/индекс электромотора, если известна
  rangeKm: number; // запас хода на одной электротяге (CLTC)
  totalRangeKm: number; // общий запас хода — для BEV равен rangeKm, для EREV/PHEV включает пробег на ДВС
  powerHp: number;
  powerKw: number;
  powerP30Kw?: number; // P30 — максимальная 30-минутная мощность (кВт), только для расчёта таможни, в характеристиках не показываем
  icePowerKw?: number; // мощность бензинового двигателя-генератора (кВт) — только для EREV, если известна
  icePowerHp?: number; // то же в л.с.
  lidar?: string; // тип и число лидаров, если есть — напр. "1 × гибридный MEMS"
  torqueNm: number;
  accelSec: number; // 0-100 км/ч
  topSpeedKmh: number;
  batteryKwh: number;
  batteryType: string; // напр. "LFP", "NMC"
  fastCharge: string; // напр. "27 мин (30→80%)"
  drive: string; // "Передний" | "Задний" | "Полный"
  highlight?: string; // короткая фраза о ключевом отличии версии
  // Габариты и масса — опционально, заполняется по мере появления данных
  lengthMm?: number;
  widthMm?: number;
  heightMm?: number;
  wheelbaseMm?: number;
  frontTrackMm?: number;
  curbWeightKg?: number; // снаряжённая масса
  grossWeightKg?: number; // полная масса
  turningRadiusM?: number; // минимальный радиус разворота
};

export type ColorOption = {
  name: string;
  hex: string;
};

export type Model = {
  slug: string;
  name: string;
  tagline: string;
  bodyType: string;
  seats: number;
  description: string;
  // версии отсортированы по цене от дешёвой к дорогой
  trims: Trim[];
  // Цвета кузова и интерьера — опционально, заполняется по мере
  // появления данных с официальных источников/китайских порталов
  exteriorColors?: ColorOption[];
  interiorColors?: ColorOption[];
};

export type Brand = {
  slug: string;
  name: string;
  country: string;
  description: string;
  accent: string; // hex, акцентный цвет бренда для мелких деталей
  logo: string; // короткая текстовая метка для плитки-логотипа (2-3 символа)
  models: Model[];
};

// Общий максимум для шкалы запаса хода на карточках (просто для наглядности)
export const RANGE_SCALE_MAX = 700;

// Единый порядок из 11 базовых характеристик — как в договорённости и на
// китайских каталогах (autohome.com.cn/config/spec/...). "Тип кузова" и
// "Мест" берутся с уровня модели, остальное — с уровня конкретной версии.
export function fullSpecRows(model: Model, trim: Trim) {
  const rows = [
    { label: "Тип кузова", value: model.bodyType },
    { label: "Мест", value: String(model.seats) },
    { label: "Привод", value: trim.drive },
    { label: "Запас хода", value: `${trim.totalRangeKm} км` },
    {
      label: "Тип двигателя",
      value: `${trim.powertrainType} — ${POWERTRAIN_LABELS[trim.powertrainType]}`,
    },
  ];
  if (trim.motorModel) {
    rows.push({ label: "Модель двигателя", value: trim.motorModel });
  }
  rows.push({
    label: "Мощность",
    value: `${trim.powerHp} л.с. (${trim.powerKw} кВт)`,
  });
  if (trim.icePowerKw !== undefined) {
    rows.push({
      label: "Мощность ДВС",
      value: `${trim.icePowerHp ?? Math.round(trim.icePowerKw * 1.35962)} л.с. (${trim.icePowerKw} кВт)`,
    });
  }
  rows.push(
    { label: "Запас хода (CLTC)", value: `${trim.rangeKm} км` },
    { label: "Максимальная скорость", value: `${trim.topSpeedKmh} км/ч` },
    { label: "Крутящий момент", value: `${trim.torqueNm} Н·м` },
    { label: "Разгон до 100 км/ч", value: `${trim.accelSec} с` },
    {
      label: "Батарея",
      value: `${trim.batteryKwh} кВт·ч (${trim.batteryType})`,
    },
    { label: "Быстрая зарядка", value: trim.fastCharge }
  );
  if (trim.lidar) {
    rows.push({ label: "Лидар", value: trim.lidar });
  }
  if (trim.lengthMm && trim.widthMm && trim.heightMm) {
    rows.push({
      label: "Длина / Ширина / Высота",
      value: `${trim.lengthMm} / ${trim.widthMm} / ${trim.heightMm} мм`,
    });
  }
  if (trim.curbWeightKg !== undefined) {
    rows.push({ label: "Вес", value: `${trim.curbWeightKg} кг` });
  }
  if (trim.grossWeightKg !== undefined) {
    rows.push({ label: "Полная масса", value: `${trim.grossWeightKg} кг` });
  }
  if (trim.wheelbaseMm !== undefined) {
    rows.push({ label: "Колёсная база", value: `${trim.wheelbaseMm} мм` });
  }
  if (trim.frontTrackMm !== undefined) {
    rows.push({ label: "Передняя колея", value: `${trim.frontTrackMm} мм` });
  }
  if (trim.turningRadiusM !== undefined) {
    rows.push({
      label: "Мин. радиус разворота",
      value: `${trim.turningRadiusM} м`,
    });
  }
  return rows;
}

export const brands: Brand[] = [
  {
    slug: "zeekr",
    name: "Zeekr",
    country: "Китай",
    accent: "#0EA5A0",
    logo: "Z",
    description:
      "Премиальный электрический бренд Geely. Собственная 800V-платформа, упор на запас хода и качество сборки.",
    models: [
      {
        slug: "zeekr-001",
        name: "Zeekr 001",
        tagline: "Лифтбек для дальних поездок",
        bodyType: "Лифтбек",
        seats: 5,
        description:
          "Флагманский лифтбек Zeekr с двумя электромоторами и запасом хода, которого хватает на дорогу между городами без дозарядки.",
        exteriorColors: [
          { name: "Полярный белый", hex: "#F2F2F0" },
          { name: "Лазерный серый", hex: "#8C9096" },
          { name: "Угольно-чёрный", hex: "#16171A" },
          { name: "Электрический синий", hex: "#1E5FBF" },
        ],
        interiorColors: [
          { name: "Чёрно-серый", hex: "#3A3A3C" },
          { name: "Чёрно-зелёный", hex: "#2E3B32" },
          { name: "Сине-белый", hex: "#D8DEE6" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 4590000,
            powertrainType: "BEV",
            rangeKm: 620,
            totalRangeKm: 620,
            powerHp: 544,
            powerKw: 400,
            torqueNm: 768,
            accelSec: 3.8,
            topSpeedKmh: 200,
            batteryKwh: 100,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Полный",
            lengthMm: 4970,
            widthMm: 1990,
            heightMm: 1560,
            wheelbaseMm: 3000,
            curbWeightKg: 2350,
            grossWeightKg: 2750,
          },
        ],
      },
      {
        slug: "zeekr-007",
        name: "Zeekr 007",
        tagline: "Спортивный седан на 800В платформе",
        bodyType: "Седан",
        seats: 5,
        description:
          "Спортивный электроседан Zeekr на 800-вольтовой платформе PMA2 — делит технику с Geely Galaxy E8. Один из самых быстрозаряжаемых седанов в классе.",
        trims: [
          {
            slug: "rwd",
            name: "RWD",
            priceFrom: 2830000,
            powertrainType: "BEV",
            rangeKm: 710,
            totalRangeKm: 710,
            powerHp: 416,
            powerKw: 310,
            torqueNm: 440,
            accelSec: 5.4,
            topSpeedKmh: 210,
            batteryKwh: 75,
            batteryType: "LFP",
            fastCharge: "~15 мин (10→80%)",
            drive: "Задний",
            highlight: "Базовая версия, задний привод",
            lidar: "1 × Hesai (стандартно)",
            lengthMm: 4865,
            widthMm: 1900,
            heightMm: 1450,
            wheelbaseMm: 2920,
            curbWeightKg: 2150,
            grossWeightKg: 2550,
          },
          {
            slug: "awd-performance",
            name: "AWD Performance",
            priceFrom: 3950000,
            powertrainType: "BEV",
            rangeKm: 630,
            totalRangeKm: 630,
            powerHp: 637,
            powerKw: 475,
            torqueNm: 686,
            accelSec: 2.84,
            topSpeedKmh: 240,
            batteryKwh: 100,
            batteryType: "NMC",
            fastCharge: "~15 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод, максимальная динамика",
            lidar: "1 × Hesai (стандартно)",
            lengthMm: 4865,
            widthMm: 1900,
            heightMm: 1450,
            wheelbaseMm: 2920,
            curbWeightKg: 2290,
            grossWeightKg: 2700,
          },
        ],
      },
      {
        slug: "zeekr-7x",
        name: "Zeekr 7X",
        tagline: "Кроссовер на платформе SEA/PMA2",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Кроссовер-версия Zeekr 007, построена на той же 800-вольтовой платформе. Один из самых продаваемых Zeekr на глобальных рынках.",
        trims: [
          {
            slug: "rwd",
            name: "RWD",
            priceFrom: 2830000,
            powertrainType: "BEV",
            rangeKm: 605,
            totalRangeKm: 605,
            powerHp: 416,
            powerKw: 310,
            torqueNm: 440,
            accelSec: 5.8,
            topSpeedKmh: 210,
            batteryKwh: 75,
            batteryType: "LFP",
            fastCharge: "~15 мин (10→80%)",
            drive: "Задний",
            lidar: "1 × Hesai, крыша (стандартно на всех версиях)",
            lengthMm: 4825,
            widthMm: 1930,
            heightMm: 1656,
            wheelbaseMm: 2925,
            curbWeightKg: 2280,
            grossWeightKg: 2700,
          },
          {
            slug: "awd-performance",
            name: "AWD Performance",
            priceFrom: 4050000,
            powertrainType: "BEV",
            rangeKm: 615,
            totalRangeKm: 615,
            powerHp: 637,
            powerKw: 475,
            torqueNm: 710,
            accelSec: 3.8,
            topSpeedKmh: 240,
            batteryKwh: 100,
            batteryType: "NMC",
            fastCharge: "~15 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод, до 802 км на топ-версии",
            lidar: "1 × Hesai, крыша (стандартно на всех версиях)",
            lengthMm: 4825,
            widthMm: 1930,
            heightMm: 1656,
            wheelbaseMm: 2925,
            curbWeightKg: 2475,
            grossWeightKg: 2900,
          },
        ],
      },
      {
        slug: "zeekr-x",
        name: "Zeekr X",
        tagline: "Компактный кроссовер",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Городской кроссовер на платформе SEA от Volvo/Geely — компактные размеры при полноценном запасе хода.",
        exteriorColors: [
          { name: "Хрустально-белый", hex: "#F1F1EC" },
          { name: "Сосново-зелёный", hex: "#3E4A3D" },
          { name: "Дворцовый бежевый", hex: "#D8CBB8" },
          { name: "Туманно-серый", hex: "#9B9C9E" },
        ],
        interiorColors: [
          { name: "Чёрный с красной прострочкой", hex: "#1A1A1C" },
          { name: "Чёрно-белый двухцветный", hex: "#E7E7E3" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 3190000,
            powertrainType: "BEV",
            rangeKm: 440,
            totalRangeKm: 440,
            powerHp: 428,
            powerKw: 315,
            torqueNm: 543,
            accelSec: 3.7,
            topSpeedKmh: 190,
            batteryKwh: 66,
            batteryType: "NMC",
            fastCharge: "~27 мин (10→80%)",
            drive: "Полный",
            lengthMm: 4450,
            widthMm: 1836,
            heightMm: 1572,
            wheelbaseMm: 2750,
            curbWeightKg: 1945,
            grossWeightKg: 2345,
          },
        ],
      },
    ],
  },
  {
    slug: "nio",
    name: "NIO",
    country: "Китай",
    accent: "#2F5DFF",
    logo: "NIO",
    description:
      "Технологичный бренд с акцентом на автопилот и сеть станций замены батарей.",
    models: [
      {
        slug: "nio-et5",
        name: "NIO ET5",
        tagline: "Электрический седан бизнес-класса",
        bodyType: "Седан",
        seats: 5,
        description:
          "Седан NIO ET5 сочетает спортивную динамику с продуманным салоном и системой автопилота NAD.",
        exteriorColors: [
          { name: "Облачно-белый", hex: "#F0F1EE" },
          { name: "Звёздно-серый", hex: "#8A8D91" },
          { name: "Глубокий чёрный", hex: "#17181B" },
          { name: "Марсианский красный", hex: "#A6362E" },
          { name: "Небесно-синий", hex: "#3E6FA8" },
        ],
        interiorColors: [
          { name: "Ониксовый чёрный", hex: "#232323" },
          { name: "Терракотовый", hex: "#B4562F" },
          { name: "Каменно-серый", hex: "#8D8A82" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 4290000,
            powertrainType: "BEV",
            rangeKm: 560,
            totalRangeKm: 560,
            powerHp: 408,
            powerKw: 300,
            torqueNm: 700,
            accelSec: 4.3,
            topSpeedKmh: 200,
            batteryKwh: 75,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Полный",
            lidar: "1 × Innovusion Falcon",
            lengthMm: 4790,
            widthMm: 1960,
            heightMm: 1499,
            wheelbaseMm: 2888,
            curbWeightKg: 2145,
            grossWeightKg: 2550,
          },
        ],
      },
      {
        slug: "nio-es6",
        name: "NIO ES6",
        tagline: "Семейный электрокроссовер",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Просторный кроссовер для семьи с большим багажником и мягкой пневмоподвеской.",
        exteriorColors: [
          { name: "Облачно-белый", hex: "#F0F1EE" },
          { name: "Глубокий чёрный", hex: "#17181B" },
          { name: "Звёздно-серый", hex: "#8A8D91" },
          { name: "Марсианский красный", hex: "#A6362E" },
          { name: "Арктический зелёный", hex: "#5A7A6A" },
        ],
        interiorColors: [
          { name: "Ониксовый чёрный", hex: "#232323" },
          { name: "Речной синий", hex: "#3D5A73" },
          { name: "Красная глина", hex: "#8C4A38" },
          { name: "Звёздный фиолетовый", hex: "#5C4A63" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 4890000,
            powertrainType: "BEV",
            rangeKm: 500,
            totalRangeKm: 500,
            powerHp: 435,
            powerKw: 320,
            torqueNm: 610,
            accelSec: 4.5,
            topSpeedKmh: 200,
            batteryKwh: 75,
            batteryType: "NMC",
            fastCharge: "~32 мин (10→80%)",
            drive: "Полный",
            lidar: "1 × Innovusion Falcon",
            lengthMm: 4854,
            widthMm: 1995,
            heightMm: 1703,
            wheelbaseMm: 2915,
            curbWeightKg: 2353,
            grossWeightKg: 2753,
          },
        ],
      },
      {
        slug: "nio-et7",
        name: "NIO ET7",
        tagline: "Флагманский седан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Оригинальный флагманский седан NIO — до 150 кВт·ч батареи и запаса хода свыше 1000 км, полный набор автопилота Aquila с 33 сенсорами.",
        trims: [
          {
            slug: "100kwh",
            name: "100 кВт·ч",
            priceFrom: 4460000,
            powertrainType: "BEV",
            rangeKm: 675,
            totalRangeKm: 675,
            powerHp: 653,
            powerKw: 480,
            torqueNm: 850,
            accelSec: 3.8,
            topSpeedKmh: 200,
            batteryKwh: 100,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Полный",
            lidar: "1 × Innovusion Falcon (часть системы Aquila)",
            lengthMm: 5101,
            widthMm: 1987,
            heightMm: 1505,
            wheelbaseMm: 3060,
            curbWeightKg: 2349,
            grossWeightKg: 2750,
          },
          {
            slug: "150kwh",
            name: "150 кВт·ч",
            priceFrom: 5220000,
            powertrainType: "BEV",
            rangeKm: 1050,
            totalRangeKm: 1050,
            powerHp: 653,
            powerKw: 480,
            torqueNm: 850,
            accelSec: 3.8,
            topSpeedKmh: 200,
            batteryKwh: 150,
            batteryType: "NMC (полутвердотельная)",
            fastCharge: "~35 мин (10→80%)",
            drive: "Полный",
            highlight: "Увеличенная батарея, запас хода свыше 1000 км",
            lidar: "1 × Innovusion Falcon (часть системы Aquila)",
            lengthMm: 5101,
            widthMm: 1987,
            heightMm: 1505,
            wheelbaseMm: 3060,
            curbWeightKg: 2379,
            grossWeightKg: 2780,
          },
        ],
      },
      {
        slug: "nio-es8",
        name: "NIO ES8",
        tagline: "Флагманский SUV на 6-7 мест",
        bodyType: "Полноразмерный SUV",
        seats: 6,
        description:
          "Новое поколение флагманского SUV NIO — сейчас главный продукт бренда по продажам. Доступен в 6- и 7-местной конфигурации.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 5490000,
            powertrainType: "BEV",
            rangeKm: 630,
            totalRangeKm: 630,
            powerHp: 691,
            powerKw: 508,
            torqueNm: 850,
            accelSec: 4.1,
            topSpeedKmh: 200,
            batteryKwh: 100,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Полный",
            lidar: "1 × Innovusion Falcon (часть системы Aquila)",
            lengthMm: 5280,
            widthMm: 2010,
            heightMm: 1800,
            wheelbaseMm: 3130,
            curbWeightKg: 2630,
            grossWeightKg: 3030,
          },
        ],
      },
    ],
  },
  {
    slug: "byd",
    name: "BYD",
    country: "Китай",
    accent: "#FF6B3D",
    logo: "BYD",
    description:
      "Крупнейший производитель электромобилей в мире. Собственные батареи Blade и широкая линейка моделей.",
    models: [
      {
        slug: "byd-han",
        name: "BYD Han EV",
        tagline: "Флагманский седан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Флагман BYD с фирменной батареей Blade — повышенная безопасность при аварии и долгий ресурс.",
        exteriorColors: [
          { name: "Чёрный", hex: "#171717" },
          { name: "Серый", hex: "#7C7E82" },
          { name: "Снежно-белый", hex: "#EFEFEC" },
          { name: "Красный", hex: "#A32A2E" },
          { name: "Синий (Аврора)", hex: "#2B4FA0" },
        ],
        interiorColors: [
          { name: "Коричневый (Кирин)", hex: "#5A3A2A" },
          { name: "Красный (Феникс)", hex: "#7A2027" },
          { name: "Серый (Небесный свод)", hex: "#8A8C8F" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 3690000,
            powertrainType: "BEV",
            rangeKm: 520,
            totalRangeKm: 520,
            powerHp: 517,
            powerKw: 380,
            torqueNm: 700,
            accelSec: 3.9,
            topSpeedKmh: 185,
            batteryKwh: 85.4,
            batteryType: "LFP",
            fastCharge: "~28 мин (10→80%)",
            drive: "Полный",
            lengthMm: 4996,
            widthMm: 1910,
            heightMm: 1496,
            wheelbaseMm: 2921,
            curbWeightKg: 2250,
            grossWeightKg: 2660,
          },
        ],
      },
      {
        slug: "byd-song-plus",
        name: "BYD Song Plus EV",
        tagline: "Кроссовер на каждый день",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Один из самых популярных электрокроссоверов BYD — баланс цены, запаса хода и практичности.",
        exteriorColors: [
          { name: "Снежно-белый", hex: "#EFEFEC" },
          { name: "Каменно-серый", hex: "#8A8C8F" },
          { name: "Чёрный", hex: "#1A1A1A" },
          { name: "Красный", hex: "#A32A2E" },
        ],
        interiorColors: [
          { name: "Чёрный", hex: "#1E1E1E" },
          { name: "Светло-бежевый", hex: "#DCD3C4" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 2790000,
            powertrainType: "BEV",
            rangeKm: 450,
            totalRangeKm: 450,
            powerHp: 204,
            powerKw: 150,
            torqueNm: 310,
            accelSec: 8.5,
            topSpeedKmh: 175,
            batteryKwh: 71.8,
            batteryType: "LFP",
            fastCharge: "~35 мин (10→80%)",
            drive: "Передний",
            lengthMm: 4705,
            widthMm: 1890,
            heightMm: 1680,
            wheelbaseMm: 2765,
            curbWeightKg: 2295,
            grossWeightKg: 2695,
          },
        ],
      },
      {
        slug: "byd-seal",
        name: "BYD Seal",
        tagline: "Спортивный электроседан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Спортивный седан BYD на платформе e-Platform 3.0 с батареей Blade, встроенной в силовую структуру кузова (CTB-технология).",
        trims: [
          {
            slug: "premium",
            name: "Premium",
            priceFrom: 2990000,
            powertrainType: "BEV",
            rangeKm: 650,
            totalRangeKm: 650,
            powerHp: 313,
            powerKw: 230,
            torqueNm: 360,
            accelSec: 7.5,
            topSpeedKmh: 180,
            batteryKwh: 82.56,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Задний",
            highlight: "Задний привод, максимальный запас хода",
            lengthMm: 4800,
            widthMm: 1875,
            heightMm: 1460,
            wheelbaseMm: 2920,
            curbWeightKg: 2050,
            grossWeightKg: 2450,
          },
          {
            slug: "performance",
            name: "Performance",
            priceFrom: 3390000,
            powertrainType: "BEV",
            rangeKm: 580,
            totalRangeKm: 580,
            powerHp: 523,
            powerKw: 390,
            torqueNm: 670,
            accelSec: 3.8,
            topSpeedKmh: 180,
            batteryKwh: 82.56,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод, максимальная динамика",
            lengthMm: 4800,
            widthMm: 1875,
            heightMm: 1460,
            wheelbaseMm: 2920,
            curbWeightKg: 2185,
            grossWeightKg: 2585,
          },
        ],
      },
      {
        slug: "byd-atto-3",
        name: "BYD Atto 3",
        tagline: "Компактный кроссовер (Yuan Plus)",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Один из самых массовых электрокроссоверов BYD в мире, известен на домашнем рынке как Yuan Plus. Фирменный «гитарный» декор в дверных нишах.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 1690000,
            powertrainType: "BEV",
            rangeKm: 410,
            totalRangeKm: 410,
            powerHp: 204,
            powerKw: 150,
            torqueNm: 310,
            accelSec: 7.3,
            topSpeedKmh: 160,
            batteryKwh: 49.92,
            batteryType: "LFP",
            fastCharge: "~35 мин (10→80%)",
            drive: "Передний",
            lengthMm: 4455,
            widthMm: 1875,
            heightMm: 1615,
            wheelbaseMm: 2720,
            curbWeightKg: 1750,
            grossWeightKg: 2150,
          },
          {
            slug: "extended",
            name: "Extended Range",
            priceFrom: 1890000,
            powertrainType: "BEV",
            rangeKm: 480,
            totalRangeKm: 480,
            powerHp: 204,
            powerKw: 150,
            torqueNm: 310,
            accelSec: 7.3,
            topSpeedKmh: 160,
            batteryKwh: 60.48,
            batteryType: "LFP",
            fastCharge: "~35 мин (10→80%)",
            drive: "Передний",
            highlight: "Увеличенная батарея",
            lengthMm: 4455,
            widthMm: 1875,
            heightMm: 1615,
            wheelbaseMm: 2720,
            curbWeightKg: 1800,
            grossWeightKg: 2200,
          },
        ],
      },
      {
        slug: "byd-dolphin",
        name: "BYD Dolphin",
        tagline: "Городской электрохэтчбек",
        bodyType: "Хэтчбек",
        seats: 5,
        description:
          "Компактный доступный хэтчбек BYD с фирменным дизайном «в стиле дельфина» — одна из самых продаваемых моделей бренда в мире.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 1290000,
            powertrainType: "BEV",
            rangeKm: 340,
            totalRangeKm: 340,
            powerHp: 95,
            powerKw: 70,
            torqueNm: 180,
            accelSec: 12.3,
            topSpeedKmh: 130,
            batteryKwh: 44.9,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Передний",
            lengthMm: 4280,
            widthMm: 1770,
            heightMm: 1570,
            wheelbaseMm: 2700,
            curbWeightKg: 1330,
            grossWeightKg: 1650,
          },
          {
            slug: "premium",
            name: "Premium",
            priceFrom: 1590000,
            powertrainType: "BEV",
            rangeKm: 480,
            totalRangeKm: 480,
            powerHp: 204,
            powerKw: 150,
            torqueNm: 310,
            accelSec: 7.0,
            topSpeedKmh: 160,
            batteryKwh: 60.48,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Передний",
            highlight: "Увеличенная батарея и мощность",
            lengthMm: 4280,
            widthMm: 1770,
            heightMm: 1570,
            wheelbaseMm: 2700,
            curbWeightKg: 1550,
            grossWeightKg: 1900,
          },
        ],
      },
    ],
  },
  {
    slug: "toyota",
    name: "Toyota",
    country: "Китай (СП FAW-Toyota)",
    accent: "#C81E3A",
    logo: "T",
    description:
      "Электрические модели линейки bZ, которые Toyota выпускает специально для китайского рынка совместно с FAW. В bZ5 используется батарея BYD Blade.",
    models: [
      {
        slug: "toyota-bz5",
        name: "Toyota bZ5",
        tagline: "Купе-кроссовер на платформе e-TNGA",
        bodyType: "Купе-кроссовер",
        seats: 5,
        description:
          "Электрический купе-кроссовер Toyota, выпускаемый совместным предприятием FAW-Toyota. Батарея LFP Blade поставляется BYD, что даёт большой ресурс и повышенную безопасность. Все версии используют один и тот же мотор 200 кВт (268 л.с.), различия — в батарее, оснащении и системах помощи водителю.",
        exteriorColors: [
          { name: "Чёрный оникс", hex: "#0B0B0D" },
          { name: "Ртутный серебристый", hex: "#B7BCC0" },
          { name: "Ослепительно-жёлтый", hex: "#F2C22E" },
          { name: "Платиново-белый", hex: "#F1F0EA" },
          { name: "Огненно-красный", hex: "#C31E2A" },
        ],
        interiorColors: [
          { name: "Чёрный с золотистым акцентом", hex: "#2B2621" },
          { name: "Огненно-красный", hex: "#7A2020" },
          { name: "Лунно-белый", hex: "#E8E4DC" },
        ],
        // Цены в юанях переведены в рубли ОРИЕНТИРОВОЧНО по грубому курсу,
        // это китайская розничная цена без учёта доставки/растаможки в РФ.
        // Обязательно подтвердить реальную цену у поставщика перед публикацией.
        trims: [
          {
            slug: "550-joy",
            name: "550 Joy",
            priceFrom: 1750000,
            priceCny: 129800,
            powertrainType: "BEV",
            rangeKm: 550,
            totalRangeKm: 550,
            powerHp: 268,
            powerKw: 200,
            torqueNm: 330,
            accelSec: 7.5,
            topSpeedKmh: 160,
            batteryKwh: 65.28,
            batteryType: "LFP (BYD Blade)",
            fastCharge: "27 мин (30→80%)",
            drive: "Передний",
            highlight: "Базовая версия: экран 15,6\", 5G, голосовой ассистент",
            lengthMm: 4780,
            widthMm: 1866,
            heightMm: 1510,
            wheelbaseMm: 2880,
            curbWeightKg: 1950,
            grossWeightKg: 2365,
          },
          {
            slug: "550-pro",
            name: "550 Pro",
            priceFrom: 1890000,
            priceCny: 139800,
            powertrainType: "BEV",
            rangeKm: 550,
            totalRangeKm: 550,
            powerHp: 268,
            powerKw: 200,
            torqueNm: 330,
            accelSec: 7.5,
            topSpeedKmh: 160,
            batteryKwh: 65.28,
            batteryType: "LFP (BYD Blade)",
            fastCharge: "27 мин (30→80%)",
            drive: "Передний",
            highlight: "Диски 21\", LED-оптика на всю ширину, панорамная крыша",
            lengthMm: 4780,
            widthMm: 1866,
            heightMm: 1510,
            wheelbaseMm: 2880,
            curbWeightKg: 1965,
            grossWeightKg: 2380,
          },
          {
            slug: "550-pro-smart",
            name: "550 Pro Smart Edition",
            priceFrom: 2160000,
            priceCny: 159800,
            powertrainType: "BEV",
            rangeKm: 550,
            totalRangeKm: 550,
            powerHp: 268,
            powerKw: 200,
            torqueNm: 330,
            accelSec: 7.5,
            topSpeedKmh: 160,
            batteryKwh: 65.28,
            batteryType: "LFP (BYD Blade)",
            fastCharge: "27 мин (30→80%)",
            drive: "Передний",
            highlight:
              "Продвинутый автопилот Toyota Pilot (Momenta 5.0) с лидаром",
            lengthMm: 4780,
            widthMm: 1866,
            heightMm: 1510,
            wheelbaseMm: 2880,
            curbWeightKg: 1980,
            grossWeightKg: 2395,
          },
          {
            slug: "630-pro",
            name: "630 Pro",
            priceFrom: 2160000,
            priceCny: 159800,
            powertrainType: "BEV",
            rangeKm: 630,
            totalRangeKm: 630,
            powerHp: 268,
            powerKw: 200,
            torqueNm: 330,
            accelSec: 7.5,
            topSpeedKmh: 160,
            batteryKwh: 73.98,
            batteryType: "LFP (BYD Blade)",
            fastCharge: "27 мин (30→80%)",
            drive: "Передний",
            highlight: "Увеличенный запас хода 630 км, электропривод багажника",
            lengthMm: 4780,
            widthMm: 1866,
            heightMm: 1510,
            wheelbaseMm: 2880,
            curbWeightKg: 2010,
            grossWeightKg: 2425,
          },
        ],
      },
      {
        slug: "toyota-bz3",
        name: "Toyota bZ3",
        tagline: "Компактный седан на платформе e-TNGA",
        bodyType: "Седан",
        seats: 5,
        description:
          "Электрический седан Toyota, разработанный совместно с BYD в рамках СП BYD Toyota EV Technology и выпускаемый FAW-Toyota. Батарея LFP Blade и электромотор поставляются BYD, кузов и платформа — от Toyota.",
        exteriorColors: [
          { name: "Платиново-белый", hex: "#F1F0EA" },
          { name: "Кристальный серый", hex: "#8B8D8F" },
          { name: "Янтарно-коричневый", hex: "#6B4A38" },
          { name: "Слюдяно-красный", hex: "#9E2B31" },
          { name: "Чёрный", hex: "#161616" },
        ],
        interiorColors: [
          { name: "Лунно-белый", hex: "#E8E4DC" },
          { name: "Туманно-серый", hex: "#8D8D8D" },
          { name: "Золотисто-серый", hex: "#6E6660" },
          { name: "Золотисто-коричневый", hex: "#8A6A45" },
        ],
        // Цены переведены из юаней ОРИЕНТИРОВОЧНО, это китайская розничная
        // цена без учёта доставки/растаможки в РФ — подтвердить у поставщика.
        trims: [
          {
            slug: "517-joy",
            name: "517 Joy",
            priceFrom: 2290000,
            priceCny: 169800,
            powertrainType: "BEV",
            rangeKm: 517,
            totalRangeKm: 517,
            powerHp: 181,
            powerKw: 135,
            torqueNm: 303,
            accelSec: 8.5,
            topSpeedKmh: 160,
            batteryKwh: 49.9,
            batteryType: "LFP (BYD Blade)",
            fastCharge: "27 мин (30→80%)",
            drive: "Передний",
            highlight: "Базовая версия с компактной батареей",
          },
          {
            slug: "616-pro",
            name: "616 Pro",
            priceFrom: 2560000,
            priceCny: 189800,
            powertrainType: "BEV",
            rangeKm: 616,
            totalRangeKm: 616,
            powerHp: 241,
            powerKw: 180,
            torqueNm: 303,
            accelSec: 7.5,
            topSpeedKmh: 160,
            batteryKwh: 65.3,
            batteryType: "LFP (BYD Blade)",
            fastCharge: "27 мин (30→80%)",
            drive: "Передний",
            highlight: "Увеличенная батарея и более мощный мотор",
          },
          {
            slug: "616-premium",
            name: "616 Premium",
            priceFrom: 2690000,
            priceCny: 199800,
            powertrainType: "BEV",
            rangeKm: 616,
            totalRangeKm: 616,
            powerHp: 241,
            powerKw: 180,
            torqueNm: 303,
            accelSec: 7.5,
            topSpeedKmh: 160,
            batteryKwh: 65.3,
            batteryType: "LFP (BYD Blade)",
            fastCharge: "27 мин (30→80%)",
            drive: "Передний",
            highlight: "Топовая комплектация с расширенным оснащением",
          },
        ],
      },
      {
        slug: "toyota-bz3x",
        name: "Toyota bZ3X",
        tagline: "Компактный кроссовер, СП GAC-Toyota",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Третья электрическая модель Toyota в Китае — компактный кроссовер от другого совместного предприятия, GAC-Toyota. Построен на технической базе GAC Aion V, но с собственной батареей.",
        trims: [
          {
            slug: "510",
            name: "510",
            priceFrom: 1480000,
            powertrainType: "BEV",
            rangeKm: 510,
            totalRangeKm: 510,
            powerHp: 201,
            powerKw: 150,
            torqueNm: 225,
            accelSec: 8.0,
            topSpeedKmh: 160,
            batteryKwh: 58.2,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Передний",
            highlight: "Базовая версия",
            lengthMm: 4600,
            widthMm: 1875,
            heightMm: 1645,
            wheelbaseMm: 2765,
            curbWeightKg: 1730,
            grossWeightKg: 2150,
          },
          {
            slug: "610",
            name: "610",
            priceFrom: 1750000,
            powertrainType: "BEV",
            rangeKm: 610,
            totalRangeKm: 610,
            powerHp: 221,
            powerKw: 165,
            torqueNm: 225,
            accelSec: 7.6,
            topSpeedKmh: 160,
            batteryKwh: 65.4,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Передний",
            highlight: "Увеличенная батарея, лидар в топ-версии",
            lidar: "1 × лидар (топ-версия)",
            lengthMm: 4600,
            widthMm: 1875,
            heightMm: 1645,
            wheelbaseMm: 2765,
            curbWeightKg: 1835,
            grossWeightKg: 2250,
          },
        ],
      },
      {
        slug: "toyota-bz4x",
        name: "Toyota bZ4X",
        tagline: "Компактный электрокроссовер, СП FAW-Toyota",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Первый серийный электромобиль Toyota на платформе e-TNGA, родственник Lexus RZ. В Китае собирается FAW-Toyota (一汽丰田) — отдельно от bZ3/bZ5, которые делает BYD Toyota EV Technology. Батарея и электромоторы — собственной разработки Toyota, не от BYD.",
        trims: [
          {
            slug: "615-2wd",
            name: "615 2WD",
            priceCny: 240000,
            priceFrom: 3240000,
            powertrainType: "BEV",
            rangeKm: 615,
            totalRangeKm: 615,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 266,
            accelSec: 7.5,
            topSpeedKmh: 160,
            batteryKwh: 66.7,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Передний",
            highlight: "Базовая версия, передний привод",
            lengthMm: 4690,
            widthMm: 1860,
            heightMm: 1650,
            wheelbaseMm: 2850,
            curbWeightKg: 1910,
            grossWeightKg: 2310,
          },
          {
            slug: "560-4wd",
            name: "560 4WD",
            priceCny: 280000,
            priceFrom: 3780000,
            powertrainType: "BEV",
            rangeKm: 560,
            totalRangeKm: 560,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 336,
            accelSec: 6.9,
            topSpeedKmh: 160,
            batteryKwh: 72.8,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод, два электромотора",
            lengthMm: 4690,
            widthMm: 1860,
            heightMm: 1650,
            wheelbaseMm: 2850,
            curbWeightKg: 2025,
            grossWeightKg: 2425,
          },
        ],
      },
    ],
  },
  {
    slug: "xpeng",
    name: "Xpeng",
    country: "Китай",
    accent: "#00A9E0",
    logo: "X",
    description:
      "Один из лидеров по автопилоту среди китайских EV-брендов, известен собственной системой помощи водителю XNGP.",
    models: [
      {
        slug: "xpeng-g6",
        name: "Xpeng G6",
        tagline: "Купе-кроссовер с фирменным автопилотом",
        bodyType: "Купе-кроссовер",
        seats: 5,
        description:
          "Электрический купе-кроссовер на 800V-платформе SEPA 2.0 с одной из лучших в классе систем автопилота XNGP.",
        exteriorColors: [
          { name: "Арктический белый", hex: "#F1F1EE" },
          { name: "Полночный чёрный", hex: "#141416" },
          { name: "Серебристый иней", hex: "#B9BDC0" },
          { name: "Графитовый серый", hex: "#54565A" },
          { name: "Огненный оранжевый", hex: "#E0562A" },
        ],
        interiorColors: [
          { name: "Тёмно-серый", hex: "#3B3B3D" },
          { name: "Светло-серый", hex: "#C7C6C2" },
        ],
        trims: [
          {
            slug: "long-range",
            name: "Long Range",
            priceFrom: 2390000,
            powertrainType: "BEV",
            rangeKm: 580,
            totalRangeKm: 580,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 440,
            accelSec: 6.9,
            topSpeedKmh: 200,
            batteryKwh: 80.9,
            batteryType: "NMC",
            fastCharge: "20 мин (10→80%)",
            drive: "Задний",
            lidar: "1 × RoboSense (серия M)",
            lengthMm: 4753,
            widthMm: 1920,
            heightMm: 1650,
            wheelbaseMm: 2890,
            curbWeightKg: 2003,
            grossWeightKg: 2403,
          },
        ],
      },
      {
        slug: "xpeng-mona-m03",
        name: "Xpeng Mona M03",
        tagline: "Самая продаваемая модель Xpeng",
        bodyType: "Лифтбек",
        seats: 5,
        description:
          "Доступный электролифтбек, разработанный совместно с Didi — бестселлер Xpeng, обеспечивающий больше 40% продаж бренда.",
        trims: [
          {
            slug: "515",
            name: "515",
            priceFrom: 1620000,
            powertrainType: "BEV",
            rangeKm: 515,
            totalRangeKm: 515,
            powerHp: 190,
            powerKw: 140,
            torqueNm: 285,
            accelSec: 7.7,
            topSpeedKmh: 175,
            batteryKwh: 51.8,
            batteryType: "LFP",
            fastCharge: "~28 мин (10→80%)",
            drive: "Передний",
            lengthMm: 4780,
            widthMm: 1896,
            heightMm: 1445,
            wheelbaseMm: 2815,
            curbWeightKg: 1661,
            grossWeightKg: 2010,
          },
          {
            slug: "620-max",
            name: "620 Max",
            priceFrom: 1890000,
            powertrainType: "BEV",
            rangeKm: 620,
            totalRangeKm: 620,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 285,
            accelSec: 7.0,
            topSpeedKmh: 175,
            batteryKwh: 62.2,
            batteryType: "LFP",
            fastCharge: "~28 мин (10→80%)",
            drive: "Передний",
            highlight: "Увеличенная батарея, топовое оснащение",
            lengthMm: 4780,
            widthMm: 1896,
            heightMm: 1445,
            wheelbaseMm: 2815,
            curbWeightKg: 1739,
            grossWeightKg: 2090,
          },
        ],
      },
      {
        slug: "xpeng-p7",
        name: "Xpeng P7",
        tagline: "Спортивный седан на 800В платформе",
        bodyType: "Седан",
        seats: 5,
        description:
          "Спортивный электроседан второго поколения на платформе SEPA 2.0 — поддерживает сверхбыструю 5C-зарядку.",
        trims: [
          {
            slug: "rwd",
            name: "RWD",
            priceFrom: 2830000,
            powertrainType: "BEV",
            rangeKm: 807,
            totalRangeKm: 807,
            powerHp: 362,
            powerKw: 270,
            torqueNm: 440,
            accelSec: 5.7,
            topSpeedKmh: 200,
            batteryKwh: 74.9,
            batteryType: "LFP",
            fastCharge: "~12 мин (10→80%)",
            drive: "Задний",
          },
          {
            slug: "awd-performance",
            name: "AWD Performance",
            priceFrom: 3780000,
            powertrainType: "BEV",
            rangeKm: 860,
            totalRangeKm: 860,
            powerHp: 586,
            powerKw: 437,
            torqueNm: 668,
            accelSec: 3.7,
            topSpeedKmh: 200,
            batteryKwh: 92.2,
            batteryType: "NMC",
            fastCharge: "~15 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод, максимальная динамика",
          },
        ],
      },
    ],
  },
  {
    slug: "li-auto",
    name: "Li Auto",
    country: "Китай",
    accent: "#2D6A4F",
    logo: "LI",
    description:
      "Специализируется на больших семейных SUV с увеличенным запасом хода за счёт бензинового генератора (EREV).",
    models: [
      {
        slug: "li-l9",
        name: "Li Auto L9",
        tagline: "Полноразмерный SUV на 6 мест",
        bodyType: "Полноразмерный SUV",
        seats: 6,
        description:
          "Флагманский SUV Li Auto с электрическим приводом и бензиновым генератором для увеличения запаса хода — можно ездить только на электричестве в городе и не бояться дальних поездок.",
        exteriorColors: [
          { name: "Золотистый металлик", hex: "#A98A5C" },
          { name: "Серебристый металлик", hex: "#B9BABC" },
          { name: "Серый металлик", hex: "#6E7073" },
          { name: "Чёрный металлик", hex: "#161616" },
        ],
        interiorColors: [
          { name: "Чёрно-коричневый", hex: "#4A362A" },
          { name: "Чёрно-оранжевый", hex: "#B5501F" },
          { name: "Чёрно-белый", hex: "#E7E5E0" },
        ],
        trims: [
          {
            slug: "max",
            name: "Max",
            priceFrom: 4850000,
            powertrainType: "EREV",
            rangeKm: 215,
            totalRangeKm: 1300,
            powerHp: 449,
            powerKw: 330,
            torqueNm: 620,
            accelSec: 5.3,
            topSpeedKmh: 180,
            batteryKwh: 44.5,
            batteryType: "LFP",
            fastCharge: "~20 мин (10→80%)",
            drive: "Полный",
            highlight: "215 км на электротяге + генератор — общий запас свыше 1300 км",
            lidar: "1 × Hesai AT128",
            lengthMm: 5218,
            widthMm: 1998,
            heightMm: 1800,
            wheelbaseMm: 3105,
            curbWeightKg: 2545,
            grossWeightKg: 3025,
          },
        ],
      },
      {
        slug: "li-l7",
        name: "Li Auto L7",
        tagline: "Самый популярный SUV линейки Li Auto",
        bodyType: "SUV",
        seats: 5,
        description:
          "Пятиместная версия успешной формулы Li Auto: электропривод с бензиновым генератором для увеличения запаса хода.",
        trims: [
          {
            slug: "pro",
            name: "Pro",
            priceFrom: 3390000,
            powertrainType: "EREV",
            rangeKm: 210,
            totalRangeKm: 1300,
            powerHp: 340,
            powerKw: 250,
            torqueNm: 500,
            accelSec: 5.3,
            topSpeedKmh: 180,
            batteryKwh: 42.8,
            batteryType: "LFP",
            fastCharge: "~20 мин (10→80%)",
            drive: "Полный",
            highlight: "210 км на электротяге + генератор — общий запас свыше 1300 км",
            lengthMm: 5050,
            widthMm: 1995,
            heightMm: 1750,
            wheelbaseMm: 3005,
            curbWeightKg: 2450,
            grossWeightKg: 2930,
          },
        ],
      },
    ],
  },
  {
    slug: "leapmotor",
    name: "Leapmotor",
    country: "Китай",
    accent: "#5B7FDE",
    logo: "LM",
    description:
      "Бренд с упором на доступную цену без потери в оснащении, поддержан инвестициями Stellantis.",
    models: [
      {
        slug: "leapmotor-c10",
        name: "Leapmotor C10",
        tagline: "Просторный семейный кроссовер",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Среднеразмерный кроссовер с большим салоном и хорошим соотношением цены и оснащения.",
        exteriorColors: [
          { name: "Жемчужно-белый", hex: "#EFEFEA" },
          { name: "Полуночно-серый", hex: "#54565A" },
          { name: "Нефритово-зелёный", hex: "#3E5A4E" },
          { name: "Терракотовый серый", hex: "#8A7C70" },
          { name: "Металлик чёрный", hex: "#161616" },
        ],
        interiorColors: [
          { name: "Закатный оранжевый", hex: "#C05A2A" },
          { name: "Звёздный фиолетовый", hex: "#5C4D63" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 1690000,
            powertrainType: "BEV",
            rangeKm: 480,
            totalRangeKm: 480,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 320,
            accelSec: 7.5,
            topSpeedKmh: 170,
            batteryKwh: 69.9,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Задний",
            lengthMm: 4740,
            widthMm: 1900,
            heightMm: 1680,
            wheelbaseMm: 2825,
            curbWeightKg: 1940,
            grossWeightKg: 2340,
          },
          {
            slug: "210-ereev",
            name: "210 EREV",
            priceCny: 122800,
            priceFrom: 1660000,
            powertrainType: "EREV",
            rangeKm: 210,
            totalRangeKm: 1190,
            powerHp: 212,
            powerKw: 158,
            torqueNm: 320,
            accelSec: 9.0,
            topSpeedKmh: 170,
            batteryKwh: 28.4,
            batteryType: "LFP",
            fastCharge: "~20 мин (30→80%)",
            drive: "Задний",
            highlight: "Версия с бензиновым генератором 1,5 л — общий запас хода 1190 км",
            lengthMm: 4740,
            widthMm: 1900,
            heightMm: 1680,
            wheelbaseMm: 2825,
            curbWeightKg: 1850,
            grossWeightKg: 2250,
          },
        ],
      },
      {
        slug: "leapmotor-t03",
        name: "Leapmotor T03",
        tagline: "Мини-электромобиль класса A00",
        bodyType: "Хэтчбек",
        seats: 4,
        description:
          "Компактный доступный электромобиль, продаётся уже в 20+ странах мира — один из самых успешных бюджетных электромобилей Leapmotor.",
        trims: [
          {
            slug: "310",
            name: "310",
            priceFrom: 950000,
            powertrainType: "BEV",
            rangeKm: 310,
            totalRangeKm: 310,
            powerHp: 94,
            powerKw: 70,
            torqueNm: 158,
            accelSec: 12.7,
            topSpeedKmh: 130,
            batteryKwh: 29.9,
            batteryType: "LFP",
            fastCharge: "~35 мин (10→80%)",
            drive: "Передний",
          },
          {
            slug: "403",
            name: "403",
            priceFrom: 1150000,
            powertrainType: "BEV",
            rangeKm: 403,
            totalRangeKm: 403,
            powerHp: 94,
            powerKw: 70,
            torqueNm: 158,
            accelSec: 12.7,
            topSpeedKmh: 130,
            batteryKwh: 41.3,
            batteryType: "LFP",
            fastCharge: "~35 мин (10→80%)",
            drive: "Передний",
            highlight: "Увеличенная батарея, больший запас хода",
          },
        ],
      },
      {
        slug: "leapmotor-b10",
        name: "Leapmotor B10",
        tagline: "Компактный кроссовер, бестселлер бренда",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Новый компактный кроссовер Leapmotor — один из самых быстрорастущих по продажам продуктов бренда, уже поставляется и в Латинскую Америку.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 1620000,
            powertrainType: "BEV",
            rangeKm: 600,
            totalRangeKm: 600,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 240,
            accelSec: 7.8,
            topSpeedKmh: 175,
            batteryKwh: 67.1,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Задний",
          },
          {
            slug: "pro",
            name: "Pro",
            priceFrom: 1750000,
            powertrainType: "BEV",
            rangeKm: 600,
            totalRangeKm: 600,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 240,
            accelSec: 7.8,
            topSpeedKmh: 175,
            batteryKwh: 67.1,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Задний",
            highlight: "Расширенное оснащение",
          },
        ],
      },
    ],
  },
  {
    slug: "aion",
    name: "GAC Aion",
    country: "Китай",
    accent: "#7C3AED",
    logo: "AI",
    description:
      "Электрический суббренд GAC, один из крупнейших производителей EV в Китае по объёму продаж.",
    models: [
      {
        slug: "aion-y-plus",
        name: "Aion Y Plus",
        tagline: "Компактный городской кроссовер",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Доступный компактный кроссовер с хорошим запасом хода для города и пригорода.",
        exteriorColors: [
          { name: "Жемчужно-белый", hex: "#EFEFEA" },
          { name: "Элегантный серый", hex: "#8A8C8E" },
          { name: "Скоростной серебристый", hex: "#C3C5C7" },
          { name: "Полуночно-чёрный", hex: "#151515" },
          { name: "Сосново-жёлтый", hex: "#D9B24C" },
        ],
        interiorColors: [
          { name: "Чёрный", hex: "#1E1E1E" },
          { name: "Чёрный с абрикосовым", hex: "#B98A5A" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 1390000,
            powertrainType: "BEV",
            rangeKm: 510,
            totalRangeKm: 510,
            powerHp: 184,
            powerKw: 136,
            torqueNm: 225,
            accelSec: 8.0,
            topSpeedKmh: 165,
            batteryKwh: 63.2,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Передний",
            lengthMm: 4535,
            widthMm: 1870,
            heightMm: 1650,
            wheelbaseMm: 2750,
            curbWeightKg: 1700,
            grossWeightKg: 2100,
          },
        ],
      },
      {
        slug: "aion-s",
        name: "GAC Aion S",
        tagline: "Популярный электроседан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Один из самых массовых электроседанов GAC Aion — часто используется в такси и каршеринге благодаря надёжности и запасу хода.",
        trims: [
          {
            slug: "510",
            name: "510",
            priceFrom: 1550000,
            powertrainType: "BEV",
            rangeKm: 510,
            totalRangeKm: 510,
            powerHp: 181,
            powerKw: 135,
            torqueNm: 225,
            accelSec: 7.9,
            topSpeedKmh: 156,
            batteryKwh: 58.8,
            batteryType: "LFP",
            fastCharge: "~35 мин (10→80%)",
            drive: "Передний",
          },
        ],
      },
    ],
  },
  {
    slug: "voyah",
    name: "Voyah",
    country: "Китай",
    accent: "#B08D57",
    logo: "VO",
    description:
      "Премиальный бренд Dongfeng, специализируется на больших комфортных кроссоверах и минивэнах.",
    models: [
      {
        slug: "voyah-free",
        name: "Voyah Free",
        tagline: "Премиальный кроссовер-купе",
        bodyType: "Кроссовер-купе",
        seats: 5,
        description:
          "Полноприводный премиальный кроссовер с акцентом на комфорт салона и плавность хода.",
        exteriorColors: [
          { name: "Чистый белый", hex: "#EFEEE9" },
          { name: "Звёздный серебристый", hex: "#B9BABC" },
          { name: "Глубокий чёрный", hex: "#131313" },
          { name: "Рассветный красный", hex: "#A6362E" },
          { name: "Сумеречный фиолетовый", hex: "#5A4D63" },
        ],
        interiorColors: [
          { name: "Рассветный оранжевый", hex: "#C0602E" },
          { name: "Лунная тень (чёрный)", hex: "#1D1D1D" },
          { name: "Светлое молочно-белое", hex: "#E9E4D8" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 3290000,
            powertrainType: "BEV",
            rangeKm: 505,
            totalRangeKm: 505,
            powerHp: 435,
            powerKw: 320,
            torqueNm: 620,
            accelSec: 4.7,
            topSpeedKmh: 200,
            batteryKwh: 84,
            batteryType: "NMC",
            fastCharge: "~28 мин (10→80%)",
            drive: "Полный",
          },
          {
            slug: "318-ereev",
            name: "318 EREV",
            priceCny: 228900,
            priceFrom: 3090000,
            powertrainType: "EREV",
            rangeKm: 318,
            totalRangeKm: 1050,
            powerHp: 272,
            powerKw: 200,
            icePowerKw: 110,
            icePowerHp: 150,
            torqueNm: 410,
            accelSec: 7.6,
            topSpeedKmh: 180,
            batteryKwh: 43,
            batteryType: "NMC (CATL)",
            fastCharge: "~24 мин (30→80%)",
            drive: "Задний",
            highlight: "Рекордный для EREV запас хода на электротяге — 318 км",
          },
          {
            slug: "318-ereev-4wd",
            name: "318 EREV 4WD",
            priceCny: 266900,
            priceFrom: 3600000,
            powertrainType: "EREV",
            rangeKm: 259,
            totalRangeKm: 1000,
            powerHp: 483,
            powerKw: 355,
            icePowerKw: 110,
            icePowerHp: 150,
            torqueNm: 720,
            accelSec: 4.5,
            topSpeedKmh: 200,
            batteryKwh: 43,
            batteryType: "NMC (CATL)",
            fastCharge: "~24 мин (30→80%)",
            drive: "Полный",
            highlight: "Полный привод, пневмоподвеска, максимальная динамика",
          },
        ],
      },
      {
        slug: "voyah-passion",
        name: "Voyah Passion",
        tagline: "Премиальный электроседан с автопилотом Huawei",
        bodyType: "Седан",
        seats: 5,
        description:
          "Электроседан Voyah с системой автопилота Huawei ADS и продвинутыми лидарными датчиками.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 3450000,
            powertrainType: "BEV",
            rangeKm: 700,
            totalRangeKm: 700,
            powerHp: 402,
            powerKw: 300,
            torqueNm: 500,
            accelSec: 5.8,
            topSpeedKmh: 200,
            batteryKwh: 82,
            batteryType: "NMC",
            fastCharge: "~28 мин (10→80%)",
            drive: "Полный",
            lengthMm: 5088,
            widthMm: 1970,
            heightMm: 1510,
            wheelbaseMm: 3000,
            curbWeightKg: 2245,
            grossWeightKg: 2645,
          },
        ],
      },
    ],
  },
  {
    slug: "hongqi",
    name: "Hongqi",
    country: "Китай",
    accent: "#A6172A",
    logo: "HQ",
    description:
      "Исторический китайский бренд (FAW), сейчас активно развивает электрическую линейку.",
    models: [
      {
        slug: "hongqi-eqm5",
        name: "Hongqi E-QM5",
        tagline: "Доступный электроседан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Практичный электроседан от одного из старейших автомобильных брендов Китая.",
        exteriorColors: [
          { name: "Арктический белый", hex: "#EEEDE8" },
          { name: "Квантовый серебристо-серый", hex: "#AEB0B2" },
          { name: "Платиново-золотой", hex: "#B9A06A" },
          { name: "Красный флага", hex: "#A32A2E" },
          { name: "Полночный чёрный", hex: "#131313" },
        ],
        interiorColors: [
          { name: "Чёрный", hex: "#1E1E1E" },
          { name: "Какао-коричневый", hex: "#5C4534" },
          { name: "Слоновая кость", hex: "#EDE6D8" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 1590000,
            powertrainType: "BEV",
            rangeKm: 510,
            totalRangeKm: 510,
            powerHp: 190,
            powerKw: 140,
            torqueNm: 280,
            accelSec: 8.8,
            topSpeedKmh: 150,
            batteryKwh: 58.1,
            batteryType: "LFP",
            fastCharge: "~35 мин (10→80%)",
            drive: "Передний",
            lengthMm: 5040,
            widthMm: 1910,
            heightMm: 1569,
            wheelbaseMm: 2990,
            curbWeightKg: 1780,
            grossWeightKg: 2225,
          },
        ],
      },
      {
        slug: "hongqi-eh7",
        name: "Hongqi EH7",
        tagline: "Представительский электроседан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Более премиальный седан Hongqi — бренда, который традиционно поставляет автомобили для официальных лиц Китая.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 2890000,
            powertrainType: "BEV",
            rangeKm: 630,
            totalRangeKm: 630,
            powerHp: 292,
            powerKw: 215,
            torqueNm: 400,
            accelSec: 6.9,
            topSpeedKmh: 190,
            batteryKwh: 77,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Задний",
          },
        ],
      },
    ],
  },
  {
    slug: "xiaomi",
    name: "Xiaomi",
    country: "Китай",
    accent: "#FF6900",
    logo: "MI",
    description:
      "Технологический гигант Xiaomi вышел на авторынок в 2024 году — дебютный седан SU7 сразу стал хитом продаж.",
    models: [
      {
        slug: "xiaomi-su7",
        name: "Xiaomi SU7",
        tagline: "Спортивный электроседан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Дебютный электромобиль Xiaomi — спортивный седан с большим запасом хода и продуманной экосистемой умных функций.",
        exteriorColors: [
          { name: "Бухтовый синий", hex: "#1E4E7A" },
          { name: "Оливково-зелёный", hex: "#5A5F45" },
          { name: "Жемчужно-белый", hex: "#EDEDE8" },
          { name: "Бриллиантово-чёрный", hex: "#131313" },
          { name: "Лавовый оранжевый", hex: "#C2551F" },
        ],
        interiorColors: [
          { name: "Галактический серый", hex: "#5C5E62" },
          { name: "Сумеречный красный", hex: "#7A2F2A" },
          { name: "Туманный фиолетовый", hex: "#5A4E63" },
          { name: "Обсидиановый чёрный", hex: "#1B1B1D" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 2290000,
            powertrainType: "BEV",
            rangeKm: 700,
            totalRangeKm: 700,
            powerHp: 299,
            powerKw: 220,
            torqueNm: 400,
            accelSec: 5.3,
            topSpeedKmh: 210,
            batteryKwh: 73.6,
            batteryType: "LFP",
            fastCharge: "~25 мин (10→80%)",
            drive: "Задний",
            lengthMm: 4997,
            widthMm: 1963,
            heightMm: 1455,
            wheelbaseMm: 3000,
            curbWeightKg: 1980,
            grossWeightKg: 2380,
          },
        ],
      },
      {
        slug: "xiaomi-yu7",
        name: "Xiaomi YU7",
        tagline: "Второй электромобиль Xiaomi — премиальный SUV",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Электрический кроссовер Xiaomi на 800-вольтовой платформе, прямой конкурент Tesla Model Y. Запущен в июне 2025 года и сразу стал одним из хитов продаж бренда.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 3420000,
            priceCny: 253500,
            powertrainType: "BEV",
            rangeKm: 835,
            totalRangeKm: 835,
            powerHp: 315,
            powerKw: 235,
            torqueNm: 400,
            accelSec: 5.9,
            topSpeedKmh: 240,
            batteryKwh: 96.3,
            batteryType: "LFP",
            fastCharge: "~12 мин (10→80%)",
            drive: "Задний",
            highlight: "Задний привод, максимальный запас хода",
            lengthMm: 4999,
            widthMm: 1996,
            heightMm: 1600,
            wheelbaseMm: 3000,
            curbWeightKg: 2265,
            grossWeightKg: 2715,
          },
          {
            slug: "max",
            name: "Max",
            priceFrom: 4450000,
            priceCny: 329900,
            powertrainType: "BEV",
            rangeKm: 760,
            totalRangeKm: 760,
            powerHp: 681,
            powerKw: 508,
            torqueNm: 866,
            accelSec: 3.23,
            topSpeedKmh: 253,
            batteryKwh: 101.7,
            batteryType: "NMC",
            fastCharge: "~12 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод, максимальная динамика, лидар",
            lidar: "1 × Hesai AT128",
            lengthMm: 4999,
            widthMm: 1996,
            heightMm: 1600,
            wheelbaseMm: 3000,
            curbWeightKg: 2405,
            grossWeightKg: 2855,
          },
        ],
      },
    ],
  },
  {
    slug: "deepal",
    name: "Deepal",
    country: "Китай",
    accent: "#1D4ED8",
    logo: "DP",
    description:
      "Электрический суббренд Changan, ориентирован на технологичные кроссоверы среднего сегмента.",
    models: [
      {
        slug: "deepal-s07",
        name: "Deepal S07",
        tagline: "Технологичный купе-кроссовер",
        bodyType: "Купе-кроссовер",
        seats: 5,
        description:
          "Кроссовер Changan с современным салоном и хорошим соотношением цены и технологичности.",
        exteriorColors: [
          { name: "Звёздный чёрный", hex: "#141414" },
          { name: "Холодный звёздно-белый", hex: "#ECECE7" },
          { name: "Лунно-каменный серый", hex: "#8B8D8F" },
          { name: "Пылающий оранжевый", hex: "#C2551F" },
          { name: "Космический жёлтый", hex: "#D9B23C" },
          { name: "Туманно-синий", hex: "#2E5A8A" },
        ],
        interiorColors: [
          { name: "Дикая зелень", hex: "#3E5A45" },
          { name: "Теневой чёрный", hex: "#1E1E1E" },
          { name: "Волна тепла (оранжевый)", hex: "#B95A2C" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 1890000,
            powertrainType: "BEV",
            rangeKm: 552,
            totalRangeKm: 552,
            powerHp: 231,
            powerKw: 170,
            torqueNm: 320,
            accelSec: 7.5,
            topSpeedKmh: 175,
            batteryKwh: 79.97,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Задний",
            lengthMm: 4750,
            widthMm: 1930,
            heightMm: 1625,
            wheelbaseMm: 2900,
            curbWeightKg: 1920,
            grossWeightKg: 2320,
          },
          {
            slug: "ereev-215",
            name: "215 EREV",
            priceCny: 166900,
            priceFrom: 2250000,
            powertrainType: "EREV",
            rangeKm: 215,
            totalRangeKm: 1200,
            powerHp: 255,
            powerKw: 190,
            icePowerKw: 70,
            icePowerHp: 94,
            torqueNm: 320,
            accelSec: 7.5,
            topSpeedKmh: 180,
            batteryKwh: 39.05,
            batteryType: "LFP (CATL ShenXing)",
            fastCharge: "~15 мин (30→80%)",
            drive: "Задний",
            highlight: "Версия с бензиновым генератором 1,5 л, базовая батарея",
            lengthMm: 4750,
            widthMm: 1930,
            heightMm: 1625,
            wheelbaseMm: 2900,
            curbWeightKg: 2000,
            grossWeightKg: 2400,
          },
          {
            slug: "ereev-285",
            name: "285 EREV",
            priceCny: 176900,
            priceFrom: 2390000,
            powertrainType: "EREV",
            rangeKm: 285,
            totalRangeKm: 1300,
            powerHp: 255,
            powerKw: 190,
            icePowerKw: 70,
            icePowerHp: 94,
            torqueNm: 320,
            accelSec: 7.5,
            topSpeedKmh: 180,
            batteryKwh: 39.05,
            batteryType: "LFP (CATL ShenXing)",
            fastCharge: "~15 мин (30→80%)",
            drive: "Задний",
            highlight: "Версия с бензиновым генератором 1,5 л, увеличенный запас хода на электротяге",
            lengthMm: 4750,
            widthMm: 1930,
            heightMm: 1625,
            wheelbaseMm: 2900,
            curbWeightKg: 2035,
            grossWeightKg: 2435,
          },
        ],
      },
      {
        slug: "deepal-s05",
        name: "Deepal S05",
        tagline: "Компактный доступный кроссовер",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Более компактный и доступный кроссовер Deepal — младшая модель линейки, тот же фирменный стиль, что и у S07.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 1490000,
            powertrainType: "BEV",
            rangeKm: 470,
            totalRangeKm: 470,
            powerHp: 190,
            powerKw: 140,
            torqueNm: 280,
            accelSec: 8.0,
            topSpeedKmh: 170,
            batteryKwh: 62.5,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Передний",
          },
        ],
      },
    ],
  },
  {
    slug: "avatr",
    name: "Avatr",
    country: "Китай",
    accent: "#111827",
    logo: "AV",
    description:
      "Совместный премиальный бренд Changan, Huawei и CATL — упор на автопилот Huawei и премиальные технологии.",
    models: [
      {
        slug: "avatr-07",
        name: "Avatr 07",
        tagline: "Премиальный электроседан-кроссовер",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Премиальная модель с автопилотом Huawei ADS и батареей CATL.",
        exteriorColors: [
          { name: "Скальный серый", hex: "#7C7E80" },
          { name: "Галечный чёрный", hex: "#161616" },
          { name: "Рассветный белый", hex: "#ECEBE6" },
          { name: "Звёздный фиолетовый", hex: "#5A4A63" },
          { name: "Изумрудно-зелёный", hex: "#3E5A4C" },
        ],
        interiorColors: [
          { name: "Звёздная река (фиолетовый)", hex: "#4A3A52" },
          { name: "Облачные горы (коричневый)", hex: "#6E5238" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 2990000,
            powertrainType: "BEV",
            rangeKm: 550,
            totalRangeKm: 550,
            powerHp: 313,
            powerKw: 230,
            torqueNm: 420,
            accelSec: 6.9,
            topSpeedKmh: 200,
            batteryKwh: 80,
            batteryType: "NMC",
            fastCharge: "~28 мин (10→80%)",
            drive: "Задний",
            lidar: "1 × Huawei (гибридный, вращающееся зеркало)",
            lengthMm: 4825,
            widthMm: 1980,
            heightMm: 1620,
            wheelbaseMm: 2940,
            curbWeightKg: 2240,
            grossWeightKg: 2640,
          },
        ],
      },
      {
        slug: "avatr-12",
        name: "Avatr 12",
        tagline: "Флагманский седан бренда",
        bodyType: "Седан",
        seats: 5,
        description:
          "Флагманский седан Avatr — совместная разработка Changan, Huawei и CATL с полным набором автопилота Huawei ADS 3.0.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 3290000,
            powertrainType: "BEV",
            rangeKm: 555,
            totalRangeKm: 555,
            powerHp: 388,
            powerKw: 285,
            torqueNm: 550,
            accelSec: 5.4,
            topSpeedKmh: 210,
            batteryKwh: 90,
            batteryType: "NMC",
            fastCharge: "~28 мин (10→80%)",
            drive: "Полный",
            lidar: "1 × Huawei (гибридный, вращающееся зеркало)",
            lengthMm: 5020,
            widthMm: 1999,
            heightMm: 1450,
            wheelbaseMm: 3020,
            curbWeightKg: 2365,
            grossWeightKg: 2765,
          },
        ],
      },
    ],
  },
  {
    slug: "im-motors",
    name: "IM Motors",
    country: "Китай",
    accent: "#0891B2",
    logo: "IM",
    description:
      "Премиальный бренд при поддержке SAIC и Alibaba, известен как «Zhiji» на домашнем рынке.",
    models: [
      {
        slug: "im-ls6",
        name: "IM LS6",
        tagline: "Премиальный кроссовер",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Премиальный кроссовер с батареей высокой плотности и продвинутой системой помощи водителю.",
        exteriorColors: [
          { name: "Белый «Афина»", hex: "#EEEDE7" },
          { name: "Чёрный «Сезанн»", hex: "#151515" },
          { name: "Серый «Рембрандт»", hex: "#8B8C8E" },
          { name: "Бирюзовый «Моне»", hex: "#3E7A78" },
          { name: "Розовый «Фердинанд»", hex: "#C88A96" },
        ],
        interiorColors: [
          { name: "Бежевый «Дувр»", hex: "#D8CCB8" },
          { name: "Бирюзовый «Эльба»", hex: "#3E6E70" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 2690000,
            powertrainType: "BEV",
            rangeKm: 580,
            totalRangeKm: 580,
            powerHp: 292,
            powerKw: 215,
            torqueNm: 350,
            accelSec: 6.5,
            topSpeedKmh: 190,
            batteryKwh: 90,
            batteryType: "NMC",
            fastCharge: "~26 мин (10→80%)",
            drive: "Задний",
            lengthMm: 4907,
            widthMm: 1988,
            heightMm: 1669,
            wheelbaseMm: 2955,
            curbWeightKg: 1731,
            grossWeightKg: 2131,
          },
        ],
      },
      {
        slug: "im-l6",
        name: "IM L6",
        tagline: "Электроседан среднего сегмента",
        bodyType: "Седан",
        seats: 5,
        description:
          "Более доступный седан IM Motors — та же премиальная платформа, что и у LS6, но в кузове седан и по более низкой цене.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 2190000,
            powertrainType: "BEV",
            rangeKm: 620,
            totalRangeKm: 620,
            powerHp: 272,
            powerKw: 200,
            torqueNm: 320,
            accelSec: 6.5,
            topSpeedKmh: 190,
            batteryKwh: 90,
            batteryType: "NMC",
            fastCharge: "~26 мин (10→80%)",
            drive: "Задний",
          },
        ],
      },
    ],
  },
  {
    slug: "wuling",
    name: "Wuling",
    country: "Китай",
    accent: "#DC2626",
    logo: "WL",
    description:
      "Совместное предприятие SAIC-GM-Wuling, производит одни из самых доступных электромобилей в мире.",
    models: [
      {
        slug: "wuling-bingo",
        name: "Wuling Bingo",
        tagline: "Компактный городской электромобиль",
        bodyType: "Хэтчбек",
        seats: 4,
        description:
          "Один из самых доступных электромобилей на рынке — идеален для города, отличная альтернатива скутеру или второй машине в семью.",
        exteriorColors: [
          { name: "Молочно-белый", hex: "#F1EFE8" },
          { name: "Свежий синий", hex: "#5A87A8" },
          { name: "Мятно-зелёный", hex: "#7CA88C" },
          { name: "Млечный путь синий", hex: "#3E5F8A" },
          { name: "Мусс зелёный", hex: "#8CA870" },
        ],
        interiorColors: [
          { name: "Мягкий белый", hex: "#EFEBE0" },
          { name: "Тёплый коричневый", hex: "#8A6B4E" },
        ],
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 590000,
            powertrainType: "BEV",
            rangeKm: 333,
            totalRangeKm: 333,
            powerHp: 84,
            powerKw: 62,
            torqueNm: 150,
            accelSec: 13.0,
            topSpeedKmh: 105,
            batteryKwh: 31.7,
            batteryType: "LFP",
            fastCharge: "~35 мин (10→80%)",
            drive: "Передний",
            lengthMm: 3950,
            widthMm: 1708,
            heightMm: 1580,
            wheelbaseMm: 2560,
            curbWeightKg: 1050,
            grossWeightKg: 1400,
          },
        ],
      },
      {
        slug: "wuling-hongguang-mini-ev",
        name: "Wuling Hongguang Mini EV",
        tagline: "Легендарный сверхбюджетный электромобиль",
        bodyType: "Мини-хэтчбек",
        seats: 4,
        description:
          "Один из самых продаваемых электромобилей в истории — простая, надёжная и очень доступная городская модель, разошедшаяся тиражом более 1 млн штук.",
        trims: [
          {
            slug: "170",
            name: "170",
            priceFrom: 490000,
            powertrainType: "BEV",
            rangeKm: 170,
            totalRangeKm: 170,
            powerHp: 27,
            powerKw: 20,
            torqueNm: 85,
            accelSec: 15.0,
            topSpeedKmh: 100,
            batteryKwh: 13.9,
            batteryType: "LFP",
            fastCharge: "~1 час (30→80%)",
            drive: "Задний",
            lengthMm: 3059,
            widthMm: 1521,
            heightMm: 1614,
            wheelbaseMm: 2010,
            curbWeightKg: 665,
            grossWeightKg: 950,
          },
          {
            slug: "300",
            name: "300",
            priceFrom: 650000,
            powertrainType: "BEV",
            rangeKm: 300,
            totalRangeKm: 300,
            powerHp: 27,
            powerKw: 20,
            torqueNm: 85,
            accelSec: 15.0,
            topSpeedKmh: 100,
            batteryKwh: 26.5,
            batteryType: "LFP",
            fastCharge: "~1 час (30→80%)",
            drive: "Задний",
            highlight: "Увеличенная батарея, больший запас хода",
            lengthMm: 3059,
            widthMm: 1521,
            heightMm: 1614,
            wheelbaseMm: 2010,
            curbWeightKg: 750,
            grossWeightKg: 1035,
          },
        ],
      },
    ],
  },
  {
    slug: "audi",
    name: "Audi",
    country: "Китай (СП SAIC-VW)",
    accent: "#BB0A30",
    logo: "A",
    description:
      "Электромобиль Q5 e-tron создан специально для китайского рынка на платформе MEB группы Volkswagen и выпускается совместным предприятием SAIC-VW.",
    models: [
      {
        slug: "audi-q5-e-tron",
        name: "Audi Q5 e-tron",
        tagline: "Премиальный кроссовер для китайского рынка",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Удлинённая версия Q4 e-tron, разработанная специально для Китая. Построена на платформе MEB, разделяет технику с Volkswagen ID.6 и ID.4.",
        trims: [
          {
            slug: "40-e-tron",
            name: "40 e-tron",
            priceFrom: 4720000,
            powertrainType: "BEV",
            rangeKm: 550,
            totalRangeKm: 550,
            powerHp: 201,
            powerKw: 150,
            torqueNm: 310,
            accelSec: 8.4,
            topSpeedKmh: 160,
            batteryKwh: 83.4,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Задний",
            highlight: "Базовая версия, один электромотор",
            lengthMm: 4876,
            widthMm: 1860,
            heightMm: 1675,
            wheelbaseMm: 2965,
            curbWeightKg: 2225,
            grossWeightKg: 2680,
          },
          {
            slug: "50-e-tron-quattro",
            name: "50 e-tron quattro",
            priceFrom: 5800000,
            powertrainType: "BEV",
            rangeKm: 520,
            totalRangeKm: 520,
            powerHp: 305,
            powerKw: 224,
            torqueNm: 460,
            accelSec: 6.4,
            topSpeedKmh: 180,
            batteryKwh: 83.4,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод quattro, два электромотора",
            lengthMm: 4876,
            widthMm: 1860,
            heightMm: 1675,
            wheelbaseMm: 2965,
            curbWeightKg: 2310,
            grossWeightKg: 2765,
          },
        ],
      },
      {
        slug: "audi-q4-e-tron",
        name: "Audi Q4 e-tron",
        tagline: "Компактный электрический кроссовер",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Начальная модель в электрической линейке Audi, доступна и в стандартном кузове, и в варианте Sportback. Собирается в Китае на заводе FAW-Audi в Фошане.",
        trims: [
          {
            slug: "45-e-tron",
            name: "45 e-tron",
            priceFrom: 4200000,
            powertrainType: "BEV",
            rangeKm: 605,
            totalRangeKm: 605,
            powerHp: 282,
            powerKw: 210,
            torqueNm: 545,
            accelSec: 6.7,
            topSpeedKmh: 180,
            batteryKwh: 82,
            batteryType: "NMC",
            fastCharge: "~28 мин (10→80%)",
            drive: "Задний",
            highlight: "Базовая версия, задний привод",
            lengthMm: 4591,
            widthMm: 1865,
            heightMm: 1612,
            wheelbaseMm: 2765,
            curbWeightKg: 2100,
            grossWeightKg: 2550,
          },
          {
            slug: "55-e-tron-quattro",
            name: "55 e-tron quattro",
            priceFrom: 5000000,
            powertrainType: "BEV",
            rangeKm: 550,
            totalRangeKm: 550,
            powerHp: 340,
            powerKw: 250,
            torqueNm: 545,
            accelSec: 5.7,
            topSpeedKmh: 180,
            batteryKwh: 82,
            batteryType: "NMC",
            fastCharge: "~28 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод quattro",
            lengthMm: 4591,
            widthMm: 1865,
            heightMm: 1612,
            wheelbaseMm: 2765,
            curbWeightKg: 2185,
            grossWeightKg: 2635,
          },
        ],
      },
      {
        slug: "audi-q6l-e-tron",
        name: "Audi Q6L e-tron",
        tagline: "Флагманский кроссовер на платформе PPE",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Новейший электрокроссовер Audi на 800-вольтовой платформе PPE — совместная разработка с Porsche. Удлинённая версия специально для Китая, выпускается FAW-Audi.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 6300000,
            powertrainType: "BEV",
            rangeKm: 620,
            totalRangeKm: 620,
            powerHp: 286,
            powerKw: 210,
            torqueNm: 480,
            accelSec: 6.9,
            topSpeedKmh: 210,
            batteryKwh: 94.9,
            batteryType: "NMC",
            fastCharge: "~21 мин (10→80%)",
            drive: "Задний",
            lengthMm: 4884,
            widthMm: 1972,
            heightMm: 1648,
            wheelbaseMm: 3020,
            curbWeightKg: 2295,
            grossWeightKg: 2795,
          },
          {
            slug: "quattro",
            name: "Quattro",
            priceFrom: 7200000,
            powertrainType: "BEV",
            rangeKm: 580,
            totalRangeKm: 580,
            powerHp: 387,
            powerKw: 285,
            torqueNm: 645,
            accelSec: 4.9,
            topSpeedKmh: 230,
            batteryKwh: 94.9,
            batteryType: "NMC",
            fastCharge: "~21 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод quattro, максимальная динамика",
            lengthMm: 4884,
            widthMm: 1972,
            heightMm: 1648,
            wheelbaseMm: 3020,
            curbWeightKg: 2380,
            grossWeightKg: 2880,
          },
        ],
      },
      {
        slug: "audi-a6l-e-tron",
        name: "Audi A6L e-tron",
        tagline: "Первый электроседан Audi в верхнем сегменте для Китая",
        bodyType: "Седан",
        seats: 5,
        description:
          "Первый полностью электрический седан Audi на платформе PPE специально для китайского рынка — удлинённая колёсная база на 132 мм относительно глобальной версии A6 e-tron.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 6100000,
            powertrainType: "BEV",
            rangeKm: 770,
            totalRangeKm: 770,
            powerHp: 286,
            powerKw: 210,
            torqueNm: 480,
            accelSec: 6.8,
            topSpeedKmh: 210,
            batteryKwh: 94.9,
            batteryType: "NMC",
            fastCharge: "~21 мин (10→80%)",
            drive: "Задний",
            lengthMm: 5073,
            widthMm: 1942,
            heightMm: 1522,
            wheelbaseMm: 3076,
            curbWeightKg: 2210,
            grossWeightKg: 2660,
          },
        ],
      },
    ],
  },
  {
    slug: "volkswagen",
    name: "Volkswagen",
    country: "Китай (СП SAIC-VW / FAW-VW)",
    accent: "#00437A",
    logo: "VW",
    description:
      "Линейка ID. — электромобили Volkswagen на платформе MEB, которые выпускают сразу два совместных предприятия в Китае: SAIC-VW и FAW-VW.",
    models: [
      {
        slug: "id3",
        name: "Volkswagen ID.3",
        tagline: "Компактный электрохэтчбек",
        bodyType: "Хэтчбек",
        seats: 5,
        description:
          "Самый доступный электромобиль Volkswagen в Китае и самая продаваемая модель линейки ID. Выпускается SAIC-VW.",
        trims: [
          {
            slug: "smart",
            name: "Smart",
            priceFrom: 1750000,
            powertrainType: "BEV",
            rangeKm: 450,
            totalRangeKm: 450,
            powerHp: 170,
            powerKw: 125,
            torqueNm: 310,
            accelSec: 7.6,
            topSpeedKmh: 160,
            batteryKwh: 53.6,
            batteryType: "LFP",
            fastCharge: "~48 мин (30→80%)",
            drive: "Задний",
            lengthMm: 4261,
            widthMm: 1809,
            heightMm: 1562,
            wheelbaseMm: 2765,
            curbWeightKg: 1730,
            grossWeightKg: 2180,
          },
          {
            slug: "pro",
            name: "Pro",
            priceFrom: 1950000,
            powertrainType: "BEV",
            rangeKm: 450,
            totalRangeKm: 450,
            powerHp: 170,
            powerKw: 125,
            torqueNm: 310,
            accelSec: 7.6,
            topSpeedKmh: 160,
            batteryKwh: 53.6,
            batteryType: "LFP",
            fastCharge: "~48 мин (30→80%)",
            drive: "Задний",
            highlight: "Расширенное оснащение, больший экран",
            lengthMm: 4261,
            widthMm: 1809,
            heightMm: 1562,
            wheelbaseMm: 2765,
            curbWeightKg: 1745,
            grossWeightKg: 2195,
          },
        ],
      },
      {
        slug: "id4",
        name: "Volkswagen ID.4",
        tagline: "Электрокроссовер бизнес-класса",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Второй по популярности электромобиль Volkswagen в Китае. Выпускается в двух версиях от разных СП: ID.4 X (SAIC-VW) и ID.4 CROZZ (FAW-VW) — технически идентичны.",
        trims: [
          {
            slug: "pro",
            name: "Pro",
            priceFrom: 2150000,
            powertrainType: "BEV",
            rangeKm: 442,
            totalRangeKm: 442,
            powerHp: 228,
            powerKw: 170,
            torqueNm: 310,
            accelSec: 8.5,
            topSpeedKmh: 160,
            batteryKwh: 55.7,
            batteryType: "LFP",
            fastCharge: "~32 мин (10→80%)",
            drive: "Задний",
            lengthMm: 4599,
            widthMm: 1852,
            heightMm: 1621,
            wheelbaseMm: 2765,
            curbWeightKg: 1985,
            grossWeightKg: 2435,
          },
          {
            slug: "pro-plus",
            name: "Pro+",
            priceFrom: 2450000,
            powertrainType: "BEV",
            rangeKm: 560,
            totalRangeKm: 560,
            powerHp: 228,
            powerKw: 170,
            torqueNm: 310,
            accelSec: 8.5,
            topSpeedKmh: 160,
            batteryKwh: 80.4,
            batteryType: "LFP",
            fastCharge: "~32 мин (10→80%)",
            drive: "Задний",
            highlight: "Увеличенная батарея, больший запас хода",
            lengthMm: 4599,
            widthMm: 1852,
            heightMm: 1621,
            wheelbaseMm: 2765,
            curbWeightKg: 2075,
            grossWeightKg: 2525,
          },
          {
            slug: "gtx",
            name: "GTX",
            priceFrom: 2650000,
            powertrainType: "BEV",
            rangeKm: 601,
            totalRangeKm: 601,
            powerHp: 313,
            powerKw: 230,
            torqueNm: 460,
            accelSec: 6.4,
            topSpeedKmh: 180,
            batteryKwh: 80.4,
            batteryType: "LFP",
            fastCharge: "~32 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод, спортивная версия",
            lengthMm: 4599,
            widthMm: 1852,
            heightMm: 1624,
            wheelbaseMm: 2765,
            curbWeightKg: 2140,
            grossWeightKg: 2590,
          },
        ],
      },
      {
        slug: "id6",
        name: "Volkswagen ID.6",
        tagline: "Семиместный семейный электрокроссовер",
        bodyType: "Кроссовер (7 мест)",
        seats: 7,
        description:
          "Самый большой электромобиль Volkswagen в Китае — три ряда сидений. Выпускается в версиях ID.6 X (SAIC-VW) и ID.6 CROZZ (FAW-VW), продаётся только в Китае.",
        trims: [
          {
            slug: "pro",
            name: "Pro",
            priceFrom: 3550000,
            powertrainType: "BEV",
            rangeKm: 460,
            totalRangeKm: 460,
            powerHp: 177,
            powerKw: 132,
            torqueNm: 310,
            accelSec: 9.6,
            topSpeedKmh: 160,
            batteryKwh: 62,
            batteryType: "NMC",
            fastCharge: "~35 мин (10→80%)",
            drive: "Задний",
            lengthMm: 4876,
            widthMm: 1860,
            heightMm: 1675,
            wheelbaseMm: 2965,
            curbWeightKg: 2200,
            grossWeightKg: 2750,
          },
          {
            slug: "pro-plus",
            name: "Pro+",
            priceFrom: 4050000,
            powertrainType: "BEV",
            rangeKm: 555,
            totalRangeKm: 555,
            powerHp: 201,
            powerKw: 150,
            torqueNm: 310,
            accelSec: 8.9,
            topSpeedKmh: 160,
            batteryKwh: 77,
            batteryType: "NMC",
            fastCharge: "~35 мин (10→80%)",
            drive: "Задний",
            lengthMm: 4876,
            widthMm: 1848,
            heightMm: 1680,
            wheelbaseMm: 2965,
            curbWeightKg: 2210,
            grossWeightKg: 2610,
          },
          {
            slug: "max-4wd",
            name: "Max 4WD",
            priceFrom: 4450000,
            powertrainType: "BEV",
            rangeKm: 617,
            totalRangeKm: 617,
            powerHp: 308,
            powerKw: 230,
            torqueNm: 470,
            accelSec: 7.3,
            topSpeedKmh: 160,
            batteryKwh: 83,
            batteryType: "NMC",
            fastCharge: "~35 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод, максимальный запас хода",
            lengthMm: 4876,
            widthMm: 1848,
            heightMm: 1680,
            wheelbaseMm: 2965,
            curbWeightKg: 2280,
            grossWeightKg: 2680,
          },
        ],
      },
      {
        slug: "id7",
        name: "Volkswagen ID.7 Vizzion",
        tagline: "Флагманский электроседан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Флагманский седан линейки ID., выпускается FAW-VW. Панорамная крыша SkyView блокирует до 99% УФ-излучения.",
        trims: [
          {
            slug: "pro",
            name: "Pro",
            priceFrom: 3100000,
            powertrainType: "BEV",
            rangeKm: 642,
            totalRangeKm: 642,
            powerHp: 204,
            powerKw: 150,
            torqueNm: 310,
            accelSec: 8.5,
            topSpeedKmh: 175,
            batteryKwh: 84.8,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Задний",
            highlight: "Базовая версия, максимальный запас хода",
            lengthMm: 4956,
            widthMm: 1862,
            heightMm: 1537,
            wheelbaseMm: 2965,
            curbWeightKg: 2122,
            grossWeightKg: 2522,
          },
          {
            slug: "4wd",
            name: "4WD",
            priceFrom: 3650000,
            powertrainType: "BEV",
            rangeKm: 570,
            totalRangeKm: 570,
            powerHp: 340,
            powerKw: 250,
            torqueNm: 679,
            accelSec: 5.4,
            topSpeedKmh: 180,
            batteryKwh: 84.8,
            batteryType: "NMC",
            fastCharge: "~30 мин (10→80%)",
            drive: "Полный",
            highlight: "Полный привод, вдвое мощнее",
            lengthMm: 4956,
            widthMm: 1862,
            heightMm: 1537,
            wheelbaseMm: 2965,
            curbWeightKg: 2200,
            grossWeightKg: 2600,
          },
        ],
      },
    ],
  },
  {
    slug: "geely",
    name: "Geely",
    country: "Китай",
    accent: "#004C97",
    logo: "GE",
    description:
      "Материнская компания Zeekr, Volvo и Lynk & Co. Под собственным именем Geely развивает суббренд Galaxy — электромобили и подключаемые гибриды среднего сегмента.",
    models: [
      {
        slug: "galaxy-e5",
        name: "Geely Galaxy E5",
        tagline: "Компактный электрокроссовер",
        bodyType: "Кроссовер",
        seats: 5,
        description:
          "Первая полностью электрическая модель суббренда Galaxy, конкурент BYD Atto 3 и GAC Aion Y. Использует фирменную батарею Aegis Short Blade.",
        trims: [
          {
            slug: "440",
            name: "440",
            priceFrom: 1450000,
            powertrainType: "BEV",
            rangeKm: 440,
            totalRangeKm: 440,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 320,
            accelSec: 6.9,
            topSpeedKmh: 180,
            batteryKwh: 49.52,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Передний",
            highlight: "Базовая версия",
            lengthMm: 4615,
            widthMm: 1901,
            heightMm: 1670,
            wheelbaseMm: 2750,
            curbWeightKg: 1560,
            grossWeightKg: 1960,
          },
          {
            slug: "530",
            name: "530",
            priceFrom: 1650000,
            powertrainType: "BEV",
            rangeKm: 530,
            totalRangeKm: 530,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 320,
            accelSec: 6.9,
            topSpeedKmh: 180,
            batteryKwh: 60.22,
            batteryType: "LFP",
            fastCharge: "~35 мин (10→80%)",
            drive: "Передний",
            lengthMm: 4615,
            widthMm: 1901,
            heightMm: 1670,
            wheelbaseMm: 2750,
            curbWeightKg: 1615,
            grossWeightKg: 2015,
          },
          {
            slug: "610",
            name: "610",
            priceFrom: 1850000,
            powertrainType: "BEV",
            rangeKm: 610,
            totalRangeKm: 610,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 320,
            accelSec: 6.9,
            topSpeedKmh: 180,
            batteryKwh: 68.4,
            batteryType: "LFP",
            fastCharge: "~40 мин (10→80%)",
            drive: "Передний",
            highlight: "Максимальный запас хода в линейке",
            lengthMm: 4615,
            widthMm: 1901,
            heightMm: 1670,
            wheelbaseMm: 2750,
            curbWeightKg: 1670,
            grossWeightKg: 2070,
          },
        ],
      },
      {
        slug: "galaxy-e8",
        name: "Geely Galaxy E8",
        tagline: "Флагманский электроседан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Флагманский седан суббренда Galaxy на 800-вольтовой платформе GEA — сверхбыстрая зарядка и мощная полноприводная версия.",
        trims: [
          {
            slug: "575",
            name: "575 Enlightened",
            priceFrom: 1980000,
            powertrainType: "BEV",
            rangeKm: 575,
            totalRangeKm: 575,
            powerHp: 313,
            powerKw: 230,
            torqueNm: 380,
            accelSec: 5.5,
            topSpeedKmh: 200,
            batteryKwh: 60,
            batteryType: "LFP",
            fastCharge: "~15 мин (10→80%)",
            drive: "Задний",
            highlight: "Задний привод, базовая версия",
            lengthMm: 5010,
            widthMm: 1920,
            heightMm: 1465,
            wheelbaseMm: 2925,
            curbWeightKg: 1905,
            grossWeightKg: 2305,
          },
          {
            slug: "620-4wd",
            name: "620 4WD Starfleet",
            priceFrom: 2650000,
            powertrainType: "BEV",
            rangeKm: 620,
            totalRangeKm: 620,
            powerHp: 637,
            powerKw: 475,
            torqueNm: 710,
            accelSec: 3.5,
            topSpeedKmh: 210,
            batteryKwh: 75,
            batteryType: "LFP",
            fastCharge: "8 мин (30→80%)",
            drive: "Полный",
            highlight: "Полный привод, лидар, 800В платформа",
            lidar: "1 × лидар, крыша",
            lengthMm: 5010,
            widthMm: 1920,
            heightMm: 1465,
            wheelbaseMm: 2925,
            curbWeightKg: 2150,
            grossWeightKg: 2550,
          },
        ],
      },
      {
        slug: "galaxy-a7-ev",
        name: "Geely Galaxy A7 EV",
        tagline: "Электрический среднеразмерный седан",
        bodyType: "Седан",
        seats: 5,
        description:
          "Полностью электрическая версия седана Galaxy A7 (у модели есть и гибридная PHEV-версия — в каталоге показана только электрическая). Построен на архитектуре GEA с фирменной батареей Golden Battery.",
        trims: [
          {
            slug: "standart",
            name: "Стандарт",
            priceFrom: 1690000,
            powertrainType: "BEV",
            rangeKm: 550,
            totalRangeKm: 550,
            powerHp: 218,
            powerKw: 160,
            torqueNm: 320,
            accelSec: 7.4,
            topSpeedKmh: 180,
            batteryKwh: 58.05,
            batteryType: "LFP",
            fastCharge: "~30 мин (10→80%)",
            drive: "Передний",
          },
        ],
      },
      {
        slug: "panda-mini-ev",
        name: "Geely Panda Mini EV",
        tagline: "Компактный городской электромобиль",
        bodyType: "Мини-хэтчбек",
        seats: 4,
        description:
          "Один из самых доступных электромобилей Geely — компактный городской автомобиль класса A00. Известен также как Geometry Panda Mini EV. Идеален как вторая машина или для узких городских улиц.",
        trims: [
          {
            slug: "120",
            name: "120",
            priceFrom: 540000,
            powertrainType: "BEV",
            rangeKm: 120,
            totalRangeKm: 120,
            powerHp: 27,
            powerKw: 20,
            torqueNm: 100,
            accelSec: 15.0,
            topSpeedKmh: 100,
            batteryKwh: 9.61,
            batteryType: "LFP",
            fastCharge: "~25 мин (30→80%)",
            drive: "Задний",
            highlight: "Базовая версия с меньшей батареей",
          },
          {
            slug: "210",
            name: "210",
            priceFrom: 770000,
            powertrainType: "BEV",
            rangeKm: 210,
            totalRangeKm: 210,
            powerHp: 41,
            powerKw: 30,
            torqueNm: 135,
            accelSec: 13.0,
            topSpeedKmh: 100,
            batteryKwh: 17,
            batteryType: "LFP",
            fastCharge: "~30 мин (30→80%)",
            drive: "Задний",
            highlight: "Увеличенная батарея (Gotion), больший запас хода",
          },
        ],
      },
    ],
  },
  {
    slug: "aito",
    name: "AITO",
    country: "Китай (Huawei + Seres)",
    accent: "#7B2D8E",
    logo: "AT",
    description:
      "Бренд, созданный совместно Huawei и Seres в рамках альянса HIMA. Специализируется на последовательных гибридах (EREV) с автопилотом Huawei ADS — один из лидеров сегмента наравне с Li Auto.",
    models: [
      {
        slug: "aito-m7",
        name: "AITO M7",
        tagline: "Семейный SUV с автопилотом Huawei",
        bodyType: "Полноразмерный SUV",
        seats: 6,
        description:
          "Один из самых продаваемых EREV в Китае. Электрический привод с бензиновым генератором, полный набор автопилота Huawei ADS.",
        trims: [
          {
            slug: "pro",
            name: "Pro",
            priceCny: 279800,
            priceFrom: 3780000,
            powertrainType: "EREV",
            rangeKm: 210,
            totalRangeKm: 1600,
            powerHp: 309,
            powerKw: 227,
            torqueNm: 314,
            accelSec: 5.5,
            topSpeedKmh: 200,
            batteryKwh: 37,
            batteryType: "NMC (CATL)",
            fastCharge: "~20 мин (30→80%)",
            drive: "Задний",
            highlight: "210 км на электротяге + генератор — общий запас свыше 1600 км",
            lengthMm: 5020,
            widthMm: 1945,
            heightMm: 1760,
            wheelbaseMm: 2820,
            curbWeightKg: 2200,
            grossWeightKg: 2650,
          },
          {
            slug: "max-long-range",
            name: "Max Long Range",
            priceCny: 299800,
            priceFrom: 4050000,
            powertrainType: "EREV",
            rangeKm: 327,
            totalRangeKm: 1690,
            powerHp: 309,
            powerKw: 227,
            torqueNm: 314,
            accelSec: 5.5,
            topSpeedKmh: 200,
            batteryKwh: 53.4,
            batteryType: "NMC (CATL)",
            fastCharge: "~20 мин (30→80%)",
            drive: "Задний",
            highlight: "Увеличенная батарея — 327 км на электротяге, общий запас свыше 1690 км",
            lengthMm: 5020,
            widthMm: 1945,
            heightMm: 1760,
            wheelbaseMm: 2820,
            curbWeightKg: 2280,
            grossWeightKg: 2730,
          },
        ],
      },
      {
        slug: "aito-m9",
        name: "AITO M9",
        tagline: "Флагманский SUV, конкурент премиальных брендов",
        bodyType: "Полноразмерный SUV",
        seats: 6,
        description:
          "Флагман альянса HIMA — трёхмоторный полный привод, автопилот Huawei ADS последнего поколения и один из самых мощных EREV на рынке.",
        trims: [
          {
            slug: "max",
            name: "Max",
            priceCny: 479800,
            priceFrom: 6480000,
            powertrainType: "EREV",
            rangeKm: 378,
            totalRangeKm: 1260,
            powerHp: 903,
            powerKw: 664,
            torqueNm: 865,
            accelSec: 4.0,
            topSpeedKmh: 251,
            batteryKwh: 75.4,
            batteryType: "NMC",
            fastCharge: "~25 мин (30→80%)",
            drive: "Полный",
            highlight: "Трёхмоторный полный привод, 378 км на электротяге, общий запас 1260 км",
            lengthMm: 5230,
            widthMm: 1999,
            heightMm: 1800,
            wheelbaseMm: 3110,
            curbWeightKg: 2700,
            grossWeightKg: 3150,
          },
        ],
      },
    ],
  },
  {
    slug: "lynk-co",
    name: "Lynk & Co",
    country: "Китай (Geely)",
    accent: "#E85D04",
    logo: "LC",
    description:
      "Молодёжный бренд Geely. Гибридная система EM-P — не последовательный гибрид (EREV): выше определённой скорости бензиновый двигатель напрямую подключается к колёсам через 3-скоростную трансмиссию DHT, поэтому такие модели классифицируются как обычный подключаемый гибрид (PHEV).",
    models: [
      {
        slug: "900",
        name: "Lynk & Co 900",
        tagline: "Флагманский шестиместный SUV с гибридом EM-P",
        bodyType: "Полноразмерный SUV",
        seats: 6,
        description:
          "Самый крупный и дорогой автомобиль бренда — трёхрядный SUV на платформе SPA Evo (развитие архитектуры Volvo). Первый серийный автомобиль в мире с чипом автопилота NVIDIA Thor.",
        // Цены и часть характеристик — из CarNewsChina и Wikipedia (модель 2026
        // года). Время быстрой зарядки в источниках дано противоречиво —
        // указано ориентировочно.
        trims: [
          {
            slug: "halo",
            name: "1.5T Halo",
            priceCny: 264800,
            priceFrom: 3575000,
            powertrainType: "PHEV",
            rangeKm: 185,
            totalRangeKm: 1355,
            powerHp: 711,
            powerKw: 530,
            torqueNm: 920,
            accelSec: 5.6,
            topSpeedKmh: 220,
            batteryKwh: 44.9,
            batteryType: "NMC (CALB)",
            fastCharge: "~30 мин (30→80%, оценка)",
            drive: "Полный",
            highlight: "Базовая версия, два электромотора + бензиновый 1.5T",
            lengthMm: 5240,
            widthMm: 1999,
            heightMm: 1810,
            wheelbaseMm: 3160,
            curbWeightKg: 2730,
            grossWeightKg: 3180,
          },
          {
            slug: "2-0t-ultra",
            name: "2.0T Ultra",
            priceCny: 310800,
            priceFrom: 4200000,
            powertrainType: "PHEV",
            rangeKm: 220,
            totalRangeKm: 1350,
            powerHp: 872,
            powerKw: 650,
            torqueNm: 1248,
            accelSec: 4.3,
            topSpeedKmh: 240,
            batteryKwh: 52.4,
            batteryType: "NMC (CATL Freevoy)",
            fastCharge: "~25 мин (30→80%, оценка)",
            drive: "Полный",
            highlight:
              "Три электромотора, бензиновый 2.0T, максимальная динамика",
            lengthMm: 5240,
            widthMm: 1999,
            heightMm: 1810,
            wheelbaseMm: 3160,
            curbWeightKg: 2850,
            grossWeightKg: 3300,
          },
        ],
      },
    ],
  },
];

export function getBrand(slug: string) {
  return brands.find((b) => b.slug === slug);
}

export function getModel(brandSlug: string, modelSlug: string) {
  const brand = getBrand(brandSlug);
  const model = brand?.models.find((m) => m.slug === modelSlug);
  return brand && model ? { brand, model } : undefined;
}

export function getTrim(brandSlug: string, modelSlug: string, trimSlug: string) {
  const found = getModel(brandSlug, modelSlug);
  const trim = found?.model.trims.find((t) => t.slug === trimSlug);
  return found && trim ? { ...found, trim } : undefined;
}

// Самая доступная версия модели — используется как "цена от" на карточках
export function baseTrim(model: Model) {
  return [...model.trims].sort((a, b) => a.priceFrom - b.priceFrom)[0];
}

export function allModelsFlat() {
  return brands.flatMap((b) => b.models.map((m) => ({ brand: b, model: m })));
}

// Топ-N моделей по параметру — для блока "Топ по параметрам" на главной.
// Для каждой модели берём её лучшую версию по этому параметру.
export function topModelsBy(
  metric: "range" | "price" | "accel",
  count = 3
) {
  const entries = brands.flatMap((b) =>
    b.models.map((m) => {
      let best: Trim;
      if (metric === "range") {
        best = [...m.trims].sort((a, b2) => b2.rangeKm - a.rangeKm)[0];
      } else if (metric === "accel") {
        best = [...m.trims].sort((a, b2) => a.accelSec - b2.accelSec)[0];
      } else {
        best = baseTrim(m);
      }
      return { brand: b, model: m, trim: best };
    })
  );

  const sorted = [...entries].sort((a, b) => {
    if (metric === "range") return b.trim.rangeKm - a.trim.rangeKm;
    if (metric === "accel") return a.trim.accelSec - b.trim.accelSec;
    return a.trim.priceFrom - b.trim.priceFrom;
  });

  return sorted.slice(0, count);
}

// Похожие версии из ДРУГИХ моделей, ближайшие по цене — для блока
// "Похожие версии" на странице конкретной версии
export function similarTrims(
  excludeModelSlug: string,
  priceFrom: number,
  count = 3
) {
  const all = brands.flatMap((b) =>
    b.models
      .filter((m) => m.slug !== excludeModelSlug)
      .flatMap((m) => m.trims.map((t) => ({ brand: b, model: m, trim: t })))
  );
  return all
    .sort(
      (a, b) =>
        Math.abs(a.trim.priceFrom - priceFrom) -
        Math.abs(b.trim.priceFrom - priceFrom)
    )
    .slice(0, count);
}
